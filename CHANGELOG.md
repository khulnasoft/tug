...
- Bug fixes and minor improvements
    ...
    - Fixed a bug causing unexpected behavior with [tug-tab](https://github.com/Aloxaf/tug-tab). Please upgrade if you use it.
- Thanks to @alexeisersun, @bitraid, @Lompik, and @fsc0 for the contributions

0.60.0
------
_Release highlights: https://khulnasoft.github.io/tug/releases/0.60.0/_

- Added `--accept-nth` for choosing output fields
  ```sh
  ps -ef | tug --multi --header-lines 1 | awk '{print $2}'
  # Becomes
  ps -ef | tug --multi --header-lines 1 --accept-nth 2

  git branch | tug | cut -c3-
  # Can be rewritten as
  git branch | tug --accept-nth -1
  ```
- `--accept-nth` and `--with-nth` now support a template that includes multiple field index expressions in curly braces
  ```sh
  echo foo,bar,baz | tug --delimiter , --accept-nth '{1}, {3}, {2}'
    # foo, baz, bar

  echo foo,bar,baz | tug --delimiter , --with-nth '{1},{3},{2},{1..2}'
    # foo,baz,bar,foo,bar
  ```
- Added `exclude` and `exclude-multi` actions for dynamically excluding items
  ```sh
  seq 100 | tug --bind 'ctrl-x:exclude'

  # 'exclude-multi' will exclude the selected items or the current item
  seq 100 | tug --multi --bind 'ctrl-x:exclude-multi'
  ```
- Preview window now prints wrap indicator when wrapping is enabled
  ```sh
  seq 100 | xargs | tug --wrap --preview 'echo {}' --preview-window wrap
  ```
- Bug fixes and improvements

0.59.0
------
_Release highlights: https://khulnasoft.github.io/tug/releases/0.59.0/_
...
- Prioritizing file name matches (#4192)
    ...
    - tug will automatically choose `path` scheme
        * when the input is a TTY device, where tug would start its built-in walker or run `$TUG_DEFAULT_COMMAND` which is usually a command for listing files,
        * but not when `reload` or `transform` action is bound to `start` event, because in that case, tug can't be sure of the input type.
- Added `--header-lines-border` to display header from `--header-lines` with a separate border
  ...
  ps -ef | tug --style full --layout reverse --header-lines 1 \
               --bind 'ctrl-r:reload(ps -ef)' --header 'Press CTRL-R to reload' \
               --header-lines-border bottom --no-list-border
  ```
- `click-header` event now sets `$TUG_CLICK_HEADER_WORD` and `$TUG_CLICK_HEADER_NTH`. You can use them to implement a clickable header for changing the search scope using the new `transform-nth` action.
  ```sh
  ...
  ps -ef | tug --style full --layout reverse --header-lines 1 \
               ...
               --bind 'click-header:transform-nth(
                         echo $TUG_CLICK_HEADER_NTH
                       )+transform-prompt(
                         echo "$TUG_CLICK_HEADER_WORD> "
                       )'
  ```
    - `$TUG_KEY` was updated to expose the type of the click. e.g. `click`, `ctrl-click`, etc. You can use it to implement a more sophisticated behavior.
...
- Added `--no-input` option to completely disable and hide the input section
  ...
  tug --header '[src] [test]' --no-input --layout reverse \
      --header-border bottom --input-border \
      --bind 'click-header:transform-search:echo ${TUG_CLICK_HEADER_WORD:1:-1}'

  # Vim-like mode switch
  tug --layout reverse-list --no-input \
      --bind 'j:down,k:up,/:show-input+unbind(j,k,/)' \
      --bind 'enter,esc,ctrl-c:transform:
        if [[ $TUG_INPUT_STATE = enabled ]]; then
          echo "rebind(j,k,/)+hide-input"
        elif [[ $TUG_KEY = enter ]]; then
          echo accept
        else
          echo abort
        fi
...
- Added `search(...)` and `transform-search(...)` action to trigger an tug search with an arbitrary query string. This can be used to extend the search syntax of tug. In the following example, tug will use the first word of the query to trigger ripgrep search, and use the rest of the query to perform tug search within the result.
  ```sh
...
- You can now bind actions to multiple keys and events at once by writing a comma-separated list of keys and events before the colon
  ...
  tug --bind 'start,ctrl-r:reload:ps -ef'
  ```
- `--min-height` option now takes a number followed by `+`, which tells tug to show at least that many items in the list section. The default value is now changed to `10+`.
  ```sh
  # You will only see the input section which takes 3 lines
  tug --style=full --height 1% --min-height 3
...
- Added `bell` action to ring the terminal bell
  ...
  tug --bind 'ctrl-y:execute-silent(echo -n {} | pbcopy)+bell'
  ```
...
0.58.0
------
_Release highlights: https://khulnasoft.github.io/tug/releases/0.58.0/_
...
<img src="https://raw.githubusercontent.com/khulnasoft/i/master/tug-4-borders.png" />

Also, tug now offers "style presets" for quick customization, which can be activated using the `--style` option.

| Preset    | Screenshot                                                                             |
| :---      | :---                                                                                   |
| `default` | <img src="https://raw.githubusercontent.com/khulnasoft/i/master/tug-style-default.png"/> |
| `full`    | <img src="https://raw.githubusercontent.com/khulnasoft/i/master/tug-style-full.png"/>    |
| `minimal` | <img src="https://raw.githubusercontent.com/khulnasoft/i/master/tug-style-minimal.png"/> |
...
- tug will now render a dashed line (`┈┈`) in each `--gap` for better visual separation.
  ```sh
  # All bash/zsh functions, highlighted
  declare -f |
    perl -0 -pe 's/^}\n/}\0/gm' |
    bat --plain --language bash --color always |
    tug --read0 --ansi --layout reverse --multi --highlight-line --gap
  ```
...
- You can specify `border-native` to `--tmux` so that native tmux border is used instead of `--border`. This can be useful if you start a different program from inside the popup.
  ```sh
  tug --tmux border-native --bind 'enter:execute:less {}'
  ```
...
- Added `change-nth` action for dynamically changing the value of the `--nth` option
  ...
  echo 'foo foobar foobarbaz' | tug --bind 'space:change-nth(2|3|)' --nth 1 -q foo
  ```
- `--nth` parts of each line can now be rendered in a different text style
  ```sh
  # nth in a different style
  ls -al | tug --nth -1 --color nth:italic
  ls -al | tug --nth -1 --color nth:reverse
  ls -al | tug --nth -1 --color nth:reverse:bold

  # Dim the other parts
  ls -al | tug --nth -1 --color nth:regular,fg:dim

  # With 'change-nth'. The current nth option is exported as $TUG_NTH.
  ps -ef | tug --reverse --header-lines 1 --header-border bottom --input-border \
             --color nth:regular,fg:dim \
             --bind 'ctrl-n:change-nth(8..|1|2|3|4|5|6|7|)' \
             --bind 'result:transform-prompt:echo "${TUG_NTH}> "'
  ```
...
- `TUG_PREVIEW_*` environment variables are exported to all child processes (#4098)
...
------
- Bug fixes and improvements
    - Fixed a race condition which would cause tug to present stale results after `reload` (#4070)
...
- Added `--gap[=N]` option to display empty lines between items.
    - This can be useful to visually separate adjacent multi-line items.
      ...
      declare -f | perl -0777 -pe 's/^}\n/}\0/gm' |
        bat --plain --language bash --color always |
        tug --read0 --ansi --reverse --multi --highlight-line --gap
      ```
    - Or just to make the list easier to read. For single-line items, you probably want to set `--color gutter:-1` as well to hide the gutter.
      ```sh
      tug --info inline-right --gap --color gutter:-1
      ```
...
0.55.0
------
_Release highlights: https://khulnasoft.github.io/tug/releases/0.55.0/_

- Added `exact-boundary-match` type to the search syntax. When a search term is single-quoted, tug will search for the exact occurrences of the string with both ends at word boundaries.
  ```sh
  tug --query "'here'" << EOF
  come here
  not there
...
- [bash] Fuzzy path completion is enabled for all commands
    ...
    - See the comment in `__tug_default_completion` function for more information
- Comments are now allowed in `$TUG_DEFAULT_OPTS` and `$TUG_DEFAULT_OPTS_FILE`
  ```sh
...
- Hyperlinks (OSC 8) are now supported in the preview window and in the main window
  ```sh
  printf '<< \e]8;;http://github.com/khulnasoft/tug\e\\Link to \e[32mfz\e[0mf\e]8;;\e\\ >>' | tug --ansi

  tug --preview "printf '<< \e]8;;http://github.com/khulnasoft/tug\e\\Link to \e[32mfz\e[0mf\e]8;;\e\\ >>'"
  ```
- The default `--ellipsis` is now `··` instead of `..`.
- [vim] A spec can have `exit` callback that is called with the exit status of tug
    - This can be used to clean up temporary resources or restore the original state when tug is closed without a selection
...
- Fixed incompatibility of adaptive height specification and 'start:reload'
  ...
  tug --height '~100%' --bind 'start:reload:seq 10'
  ```
- Environment variables are now available to `$TUG_DEFAULT_COMMAND`
  ```sh
  TUG_DEFAULT_COMMAND='echo $TUG_QUERY' tug --query foo
  ```
...
- (Windows) Reverted a mintty fix in 0.54.0
    - As a result, mouse may not work on mintty in fullscreen mode. However, tug will correctly read non-ASCII input in fullscreen mode (`--no-height`).
    - tug unfortunately cannot read non-ASCII input when not in fullscreen mode on Windows. So if you need to input non-ASCII characters, add `--no-height` to your `$TUG_DEFAULT_OPTS`.
...
- Reverted ALT-C binding of fish to use `cd` instead of `builtin cd`
    ...
    - See [#3928](https://github.com/khulnasoft/tug/pull/3928) for more information and consider helping zoxide fix the bug.

0.54.0
------
_Release highlights: https://khulnasoft.github.io/tug/releases/0.54.0/_
...
- Implemented line wrap of long items
    ...
    - `toggle-wrap` action toggles line wrap
      ```sh
      history | tug --tac --wrap --bind 'ctrl-/:toggle-wrap' --wrap-sign $'\t↳ '
      ```
    - tug by default binds `CTRL-/` and `ALT-/` to `toggle-wrap`
...
- Added `--info-command` option for customizing the info line
  ...
  tug --info-command='echo -e "\x1b[33;1m$TUG_POS\x1b[m/$TUG_INFO 💛"'
  ```
    - `$TUG_INFO` is set to the original info text
    - ANSI color codes are supported
- Pointer and marker signs can be set to empty strings
  ```sh
  # Minimal style
  tug --pointer '' --marker '' --prompt '' --info hidden
  ```
- Better cache management and improved rendering for `--tail`
- Improved `--sync` behavior
    - When `--sync` is provided, tug will not render the interface until the initial filtering and the associated actions (bound to any of `start`, `load`, `result`, or `focus`) are complete.
      ```sh
      # tug will not render intermediate states
      (sleep 1; seq 1000000; sleep 1) |
        tug --sync --query 5 --listen --bind start:up,load:up,result:up,focus:change-header:Ready
      ```
- GET endpoint is now available from `execute` and `transform` actions (it used to timeout due to lock conflict)
  ```sh
  tug --listen --sync --bind 'focus:transform-header:curl -s localhost:$TUG_PORT?limit=0 | jq .'
  ```
- Added `offset-middle` action to place the current item is in the middle of the screen
- tug will not start the initial reader when `reload` or `reload-sync` is bound to `start` event. `tug < /dev/null` or `: | tug` are no longer required and extraneous `load` event will not fire due to the empty list.
  ```sh
  # Now this will work as expected. Previously, this would print an invalid header line.
  # `tug < /dev/null` or `: | tug` would fix the problem, but then an extraneous
  # `load` event would fire and the header would be prematurely updated.
  tug --header 'Loading ...' --header-lines 1 \
...
0.53.0
------
_Release highlights: https://khulnasoft.github.io/tug/releases/0.53.0/_

- Multi-line display
    - See [Processing multi-line items](https://khulnasoft.github.io/tug/tips/processing-multi-line-items/)
    - tug can now display multi-line items
      ```sh
      # All bash functions, highlighted
      declare -f | perl -0777 -pe 's/^}\n/}\0/gm' |
        bat --plain --language bash --color always |
        tug --read0 --ansi --reverse --multi --highlight-line

      # Ripgrep multi-line output
      rg --pretty bash | perl -0777 -pe 's/\n\n/\n\0/gm' |
        tug --read0 --ansi --multi --highlight-line --reverse --tmux 70%
      ```
...
- Native tmux integration
    - Added `--tmux` option to replace tug-tmux script and simplify distribution
      ```sh
      # --tmux [center|top|bottom|left|right][,SIZE[%]][,SIZE[%]]
      # Center, 100% width and 70% height
      tug --tmux 100%,70% --border horizontal --padding 1,2
    ...
    - To use `--tmux` in Vim plugin:
      ```vim
      let g:tug_layout = { 'tmux': '100%,70%' }
      ```
- Added support for endless input streams
    - See [Browsing log stream with tug](https://khulnasoft.github.io/tug/tips/browsing-log-streams/)
    - Added `--tail=NUM` option to limit the number of items to keep in memory. This is useful when you want to browse an endless stream of data (e.g. log stream) with tug while limiting memory usage.
      ```sh
      # Interactive filtering of a log stream
      tail -f *.log | tug --tail 100000 --tac --no-sort --exact
      ```
- Better Windows Support
    - tug now works on Git bash (mintty) out of the box via winpty integration
    - Many fixes and improvements for Windows
- man page is now embedded in the binary; `tug --man` to see it
...
- Process started by `execute` action now directly writes to and reads from `/dev/tty`. Manual `/dev/tty` redirection for interactive programs is no longer required.
  ...
  ls | tug --bind 'space:execute:vim {}' > selected
  ```
- Added `print(...)` action to queue an arbitrary string to be printed on exit. This was mainly added to work around the limitation of `--expect` where it's not compatible with `--bind` on the same key and it would ignore other actions bound to it.
  ...
  tug --multi --expect ctrl-y --bind 'ctrl-y:select-all'

  # This is something you can do instead
  tug --multi --bind 'enter:print()+accept,ctrl-y:select-all+print(ctrl-y)+accept'
  ```
    - We also considered making them compatible, but realized that some users may have been relying on the current behavior.
- [`NO_COLOR`](https://no-color.org/) environment variable is now respected. If the variable is set, tug defaults to `--no-color` unless otherwise specified.
...
- Added color names for selected lines: `selected-fg`, `selected-bg`, and `selected-hl`
  ```sh
  tug --border --multi --info inline-right --layout reverse --marker ▏ --pointer ▌ --prompt '▌ '  \
      --highlight-line --color gutter:-1,selected-bg:238,selected-fg:146,current-fg:189
  ```
- Added `click-header` event that is triggered when the header section is clicked. When the event is triggered, `$TUG_CLICK_HEADER_COLUMN` and `$TUG_CLICK_HEADER_LINE` are set.
  ```sh
  fd --type f |
    tug --header $'[Files] [Directories]' --header-first \
        --bind 'click-header:transform:
          (( TUG_CLICK_HEADER_COLUMN <= 7 )) && echo "reload(fd --type f)"
          (( TUG_CLICK_HEADER_COLUMN >= 9 )) && echo "reload(fd --type d)"
        '
  ```
- Add `$TUG_COMPLETION_{DIR,PATH}_OPTS` for separately customizing the behavior of fuzzy completion
  ```sh
  # Set --walker options without 'follow' not to follow symbolic links
  TUG_COMPLETION_PATH_OPTS="--walker=file,dir,hidden"
  TUG_COMPLETION_DIR_OPTS="--walker=dir,hidden"
  ```
- Fixed Windows argument escaping
- Bug fixes and improvements
- The code was heavily refactored to allow using tug as a library in Go programs. The API is still experimental and subject to change.
    - https://gist.github.com/khulnasoft/193990b65be48a38aac6ac49d5669170

0.51.0
------
- Added a new environment variable `$TUG_POS` exported to the child processes. It's the vertical position of the cursor in the list starting from 1.
  ```sh
  # Toggle selection to the top or to the bottom
  seq 30 | tug --multi --bind 'load:pos(10)' \
    --bind 'shift-up:transform:for _ in $(seq $TUG_POS $TUG_MATCH_COUNT); do echo -n +toggle+up; done' \
    --bind 'shift-down:transform:for _ in $(seq 1 $TUG_POS); do echo -n +toggle+down; done'
  ```
- Added `--with-shell` option to start child processes with a custom shell command and flags
  ```sh
  gem list | tug --with-shell 'ruby -e' \
...
- Search performance optimization. You can observe 50%+ improvement in some scenarios.
  ...
  $ hyperfine -w 1 -L bin tug-0.49.0,tug-7ce6452,tug-a5447b8,tug '{bin} --filter "///" < $DATA | head -30'
  Summary
    tug --filter "///" < $DATA | head -30 ran
      1.16 ± 0.03 times faster than tug-a5447b8 --filter "///" < $DATA | head -30
      1.23 ± 0.03 times faster than tug-7ce6452 --filter "///" < $DATA | head -30
      1.52 ± 0.03 times faster than tug-0.49.0 --filter "///" < $DATA | head -30
  ```
- Added `jump` and `jump-cancel` events that are triggered when leaving `jump` mode
  ...
  tug --bind space:jump,jump:accept

  # Accept on jump, abort on cancel
  tug --bind space:jump,jump:accept,jump-cancel:abort

  # Change header on jump-cancel
  tug --bind 'space:change-header(Type jump label)+jump,jump-cancel:change-header:Jump cancelled'
  ```
- Added a new environment variable `$TUG_KEY` exported to the child processes. It's the name of the last key pressed.
  ```sh
  tug --bind 'space:jump,jump:accept,jump-cancel:transform:[[ $TUG_KEY =~ ctrl-c ]] && echo abort'
  ```
- tug can be built with profiling options. See [BUILD.md](BUILD.md) for more information.
- Bug fixes
...
- `--info=hidden` and `--info=inline-right` will no longer hide the horizontal separator by default. This gives you more flexibility in customizing the layout.
    ```sh
    tug --border --info=inline-right
    tug --border --info=inline-right --separator ═
    tug --border --info=inline-right --no-separator
    tug --border --info=hidden
    tug --border --info=hidden --separator ━
    tug --border --info=hidden --no-separator
    ```
- Added two environment variables exported to the child processes
    - `TUG_PREVIEW_LABEL`
    - `TUG_BORDER_LABEL`
    ```sh
    # Use the current value of $TUG_PREVIEW_LABEL to determine which actions to perform
    git ls-files |
      tug --header 'Press CTRL-P to change preview mode' \
          --bind='ctrl-p:transform:[[ $TUG_PREVIEW_LABEL =~ cat ]] \
...
0.48.1
------
- CTRL-T and ALT-C bindings can be disabled by setting `TUG_CTRL_T_COMMAND` and `TUG_ALT_C_COMMAND` to empty strings respectively when sourcing the script
    ```sh
    # bash
    TUG_CTRL_T_COMMAND= TUG_ALT_C_COMMAND= eval "$(tug --bash)"

    # zsh
    TUG_CTRL_T_COMMAND= TUG_ALT_C_COMMAND= eval "$(tug --zsh)"

    # fish
    tug --fish | TUG_CTRL_T_COMMAND= TUG_ALT_C_COMMAND= source
    ```
...
0.48.0
------
- Shell integration scripts are now embedded in the tug binary. This simplifies the distribution, and the users are less likely to have problems caused by using incompatible scripts and binaries.
    - bash
      ```sh
      # Set up tug key bindings and fuzzy completion
      eval "$(tug --bash)"
      ```
    - zsh
      ```sh
...
- Added options for customizing the behavior of the built-in walker
    ...
    - Examples
        ```sh
        # Built-in walker is only used by standalone tug when $TUG_DEFAULT_COMMAND is not set
        unset TUG_DEFAULT_COMMAND

        tug # default: --walker=file,follow,hidden --walker-root=. --walker-skip=.git,node_modules
        tug --walker=file,dir,hidden,follow --walker-skip=.git,node_modules,target

        # Walker options in $TUG_DEFAULT_OPTS
        export TUG_DEFAULT_OPTS="--walker=file,dir,hidden,follow --walker-skip=.git,node_modules,target"
        tug

        # Reading from STDIN; --walker is ignored
        seq 100 | tug --walker=dir

        # Reading from $TUG_DEFAULT_COMMAND; --walker is ignored
        export TUG_DEFAULT_COMMAND='seq 100'
        tug --walker=dir
        ```
...
- Replaced ["the default find command"][find] with a built-in directory walker to simplify the code and to achieve better performance and consistent behavior across platforms.
  This doesn't affect you if you have `$TUG_DEFAULT_COMMAND` set.
    - Breaking changes:
    ...
    - You may wonder why tug implements directory walker anyway when it's a filter program following the [Unix philosophy][unix].
      But tug has had [the walker code for years][walker] to tackle the performance problem on Windows. And I decided to use the same approach on different platforms as well for the benefits listed above.
...
- Added `$TUG_DEFAULT_OPTS_FILE` to allow managing default options in a file
    - See [#3618](https://github.com/khulnasoft/tug/pull/3618)
    - Option precedence from lower to higher
        1. Options read from `$TUG_DEFAULT_OPTS_FILE`
        1. Options from `$TUG_DEFAULT_OPTS`
        1. Options from command-line arguments
- Bug fixes and improvements

[find]: https://github.com/khulnasoft/tug/blob/0.46.1/src/constants.go#L60-L64
[walker]: https://github.com/khulnasoft/tug/pull/1847
...
- tug now exports the following environment variables to the child processes
  | Variable           | Description                                                 |
  | ---                | ---                                                         |
  | `TUG_LINES`        | Number of lines tug takes up excluding padding and margin   |
  | `TUG_COLUMNS`      | Number of columns tug takes up excluding padding and margin |
  | `TUG_TOTAL_COUNT`  | Total number of items                                       |
  | `TUG_MATCH_COUNT`  | Number of matched items                                     |
  | `TUG_SELECT_COUNT` | Number of selected items                                    |
  | `TUG_QUERY`        | Current query string                                        |
  | `TUG_PROMPT`       | Prompt string                                               |
  | `TUG_ACTION`       | The name of the last action performed                       |
  - This allows you to write sophisticated transformations like so
    ...
    transformer='
      ...
      lines=$(( TUG_LINES - TUG_MATCH_COUNT - 4 ))
      if [[ $TUG_MATCH_COUNT -eq 0 ]]; then
        echo "change-preview-window:hidden"
      elif [[ $lines -gt 3 ]]; then
        echo "change-preview-window:$lines"
      elif [[ $TUG_PREVIEW_LINES -ne 3 ]]; then
        echo "change-preview-window:3"
      fi
    '
    seq 10000 | tug --preview 'seq {} 10000' --preview-window up \
                    --bind "result:transform:$transformer" \
                    --bind "resize:transform:$transformer"
    ```
  - And we're phasing out `{tug:prompt}` and `{tug:action}`
...
- Added `transform` action to conditionally perform a series of actions
  ...
  echo -e "1. Hello\n2. Goodbye\n\n3. Exit" |
    tug --height '~100%' --reverse --header 'Select one' \
  ...
  echo -e "1. Hello\n2. Goodbye\n\n3. Exit" |
    tug --height '~100%' --reverse --header 'Select one' \
        --bind 'enter:transform:[[ -n {} ]] && echo accept || echo "change-header:Invalid selection"' \
        --bind 'focus:transform:[[ -n {} ]] && exit; [[ {tug:action} =~ up$ ]] && echo up || echo down'

  # A single key binding to toggle between modes
  fd --type file |
    tug --prompt 'Files> ' \
        --header 'CTRL-T: Switch between Files/Directories' \
        --bind 'ctrl-t:transform:[[ ! {tug:prompt} =~ Files ]] &&
                  echo "change-prompt(Files> )+reload(fd --type file)" ||
                  echo "change-prompt(Directories> )+reload(fd --type directory)"'
  ```
- Added placeholder expressions
    - `{tug:action}` - The name of the last action performed
    - `{tug:prompt}` - Prompt string (including ANSI color codes)
    - `{tug:query}` - Synonym for `{q}`
...
- Added `accept-or-print-query` action that acts like `accept` but prints the
  ...
  export TUG_CTRL_R_OPTS='--bind enter:accept-or-print-query'
  ```
  - Note that there are alternative ways to implement the same strategy
    ...
    export TUG_CTRL_R_OPTS='--bind "enter:become:if [ -z {} ]; then echo {q}; else echo {}; fi"'

    # Using the new 'transform' action
    export TUG_CTRL_R_OPTS='--bind "enter:transform:[ -z {} ] && echo print-query || echo accept"'
    ```
...
- (Experimental) Sixel image support in preview window (not available on Windows)
    - [bin/tug-preview.sh](bin/tug-preview.sh) is added to demonstrate how to
      display an image using Kitty image protocol or Sixel. You can use it
      like so:
      ```sh
      tug --preview='tug-preview.sh {}'
      ```
- (Experimental) iTerm2 inline image protocol support in preview window (not available on Windows)
  ...
  tug --preview 'imgcat -W $TUG_PREVIEW_COLUMNS -H $TUG_PREVIEW_LINES {}'
  ```
- HTTP server can be configured to accept remote connections
  ```sh
  # TUG_API_KEY is required for a non-localhost listen address
  export TUG_API_KEY="$(head -c 32 /dev/urandom | base64)"
  tug --listen 0.0.0.0:6266
  ```
    - To allow remote process execution, use `--listen-unsafe` instead
      (`execute*`, `reload*`, `become`, `preview`, `change-preview`, `transform-*`)
      ```sh
      tug --listen-unsafe 0.0.0.0:6266
      ```
- Bug fixes
...
- (Experimental) Added support for Kitty image protocol in the preview window
  ...
  tug --preview='
    if file --mime-type {} | grep -qF image/; then
      # --transfer-mode=memory is the fastest option but if you want tug to be able
      # to redraw the image on terminal resize or on 'change-preview-window',
      # you need to use --transfer-mode=stream.
      kitty icat --clear --transfer-mode=memory --unicode-placeholder --stdin=no --place=${TUG_PREVIEW_COLUMNS}x${TUG_PREVIEW_LINES}@0x0 {} | sed \$d
    else
      bat --color=always {}
    fi
  '
...
- (Experimental) `--listen` server can report program state in JSON format (`GET /`)
  ```sh
  # tug server started in "headless" mode
  tug --listen 6266 2> /dev/null
  ...
  ```
- `--listen` server can be secured by setting `$TUG_API_KEY` environment
  variable.
  ```sh
  export TUG_API_KEY="$(head -c 32 /dev/urandom | base64)"
  ...
  # Client
  curl localhost:6266 -H "x-api-key: $TUG_API_KEY" -d 'change-query(yo)'
  ```
...
- Added mouse events for `--bind`
    ...
    tug --bind 'scroll-up:up+up,scroll-down:down+down' \
...
- Added `offset-up` and `offset-down` actions
  ...
  tug --bind scroll-up:offset-up,scroll-down:offset-down \
      --bind ctrl-y:offset-up,ctrl-e:offset-down \
      --scroll-off=5
  ```
- Shell extensions
    - Updated bash completion for tug options
...
- Added new border style `thinblock` which uses symbols for legacy computing
  [one eighth block elements](https://en.wikipedia.org/wiki/Symbols_for_Legacy_Computing)
    - Similarly to `block`, this style is suitable when using a different
      ...
      ```sh
      BAT_THEME=GitHub tug --info=right --border=thinblock --preview-window=border-thinblock \
...
- Putting it altogether:
  ```sh
  tug-tmux -p 80% --padding 1,2 --preview 'bat --style=plain --color=always {}' \
...
- Added `zero` event that is triggered when there's no match
  ...
  echo $RANDOM | tug --bind 'zero:reload(echo $RANDOM)+clear-query' --height 3
  ```
- New actions
    - Added `track` action which makes tug track the current item when the
...
- Added `one` event that is triggered when there's only one match
  ...
  seq 10 | tug --bind one:accept
  ```
- Added `--track` option that makes tug track the current selection when the
  result list is updated. This can be useful when browsing logs using tug with
  sorting disabled.
  ```sh
  git log --oneline --graph --color=always | nl |
      tug --ansi --track --no-sort --layout=reverse-list
  ```
- If you use `--listen` option without a port number tug will automatically
  allocate an available port and export it as `$TUG_PORT` environment
  variable.
  ```sh
  # Automatic port assignment
  tug --listen --bind 'start:execute-silent:echo $TUG_PORT > /tmp/tug-port'

  # Say hello
  curl "localhost:$(cat /tmp/tug-port)" -d 'preview:echo Hello, tug is listening on $TUG_PORT.'
  ```
- A carriage return and a line feed character will be rendered as dim ␍ and
  ␊ respectively.
  ```sh
  printf "foo\rbar\nbaz" | tug --read0 --preview 'echo {}'
  ```
- tug will stop rendering a non-displayable characters as a space. This will
  likely cause less glitches in the preview window.
  ```sh
  tug --preview 'head -1000 /dev/random'
  ```
- Bug fixes and improvements
...
------
- New actions
    - `become(...)` - Replace the current tug process with the specified
      command using `execve(2)` system call.
      See https://github.com/khulnasoft/tug#turning-into-a-different-process for
      more information.
      ```sh
      # Open selected files in Vim
      tug --multi --bind 'enter:become(vim {+})'

      # Open the file in Vim and go to the line
      git grep --line-number . |
          tug --delimiter : --nth 3.. --bind 'enter:become(vim {1} +{2})'
      ```
...
- Added a way to customize the separator of inline info
  ```sh
  tug --info 'inline: ╱ ' --prompt '╱ ' --color prompt:bright-yellow
  ```
- New event
    - `focus` - Triggered when the focus changes due to a vertical cursor
      movement or a search result update
      ```sh
      tug --bind 'focus:transform-preview-label:echo [ {} ]' --preview 'cat {}'

      # Any action bound to the event runs synchronously and thus can make the interface sluggish
      # e.g. lolcat isn't one of the fastest programs, and every cursor movement in
      #      tug will be noticeably affected by its execution time
      tug --bind 'focus:transform-preview-label:echo [ {} ] | lolcat -f' --preview 'cat {}'

      # Beware not to introduce an infinite loop
      seq 10 | tug --bind 'focus:up' --cycle
      ```
- New actions
    - `change-border-label`
...
- Added draggable scrollbar to the main search window and the preview window
  ...
  # Customize scrollbar
  tug --scrollbar ┆ --color scrollbar:blue
  ```
- New event
    - Added `load` event that is triggered when the input stream is complete
      ...
      (seq 10; sleep 1; seq 11 20) | tug --prompt 'Loading> ' --bind 'load:change-prompt:Loaded> '

      # You can use it instead of 'start' event without `--sync` if asynchronous
      # trigger is not an issue.
      (seq 10; sleep 1; seq 11 20) | tug --bind 'load:last'
      ```
- New actions
    - Added `pos(...)` action to move the cursor to the numeric position
      ...
      seq 100 | tug --sync --bind 'start:pos(10)'
    ...
    - Added `reload-sync(...)` action which replaces the current list only after
      ...
      a while to produce the initial output and you don't want tug to run against
      an empty list while the command is running.
      ```sh
      # You can still filter and select entries from the initial list for 3 seconds
      seq 100 | tug --bind 'load:reload-sync(sleep 3; seq 1000)+unbind(load)'
      ```
    - Added `next-selected` and `prev-selected` actions to move between selected
      ...
      seq 10 | tug --multi --bind ctrl-n:next-selected,ctrl-p:prev-selected

      # Both actions respect --layout option
      seq 10 | tug --multi --bind ctrl-n:next-selected,ctrl-p:prev-selected --layout reverse
      ```
    ...
    - Added `transform-prompt(...)` action for transforming the prompt string
      ...
      tug --bind 'space:reload(ls),load:transform-prompt(printf "%s> " "$(date)")'
      ```
    - Added `transform-query(...)` action for transforming the query string using
      ...
      tug --bind 'space:transform-query(tr "[:lower:]" "[:upper:]" <<< {q})'

      # Bind it to 'change' event for automatic conversion
      tug --bind 'change:transform-query(tr "[:lower:]" "[:upper:]" <<< {q})'

      # Can only type numbers
      tug --bind 'change:transform-query(sed "s/[^0-9]//g" <<< {q})'
      ```
    - `put` action can optionally take an argument string
      ```sh
      # a will put 'alpha' on the prompt, ctrl-b will put 'bravo'
      tug --bind 'a:put+put(lpha),ctrl-b:put(bravo)'
      ```
...
- Behavior changes
    - tug will always execute the preview command if the command template
    ...
    - `double-click` will behave the same as `enter` unless otherwise specified,
      ...
      tug --bind 'enter:execute:less {}' # --bind 'double-click:execute:less {}'
      ```
    - If the color for `separator` is not specified, it will default to the
      ...
      scheme.
    - If `follow` flag is specified in `--preview-window` option, tug will
      ...
      this version, tug will resume following if the user scrolls the window
      to the bottom.
...
0.35.1
------
- Fixed a bug where tug with `--tiebreak=chunk` crashes on inverse match query
- Fixed a bug where clicking above tug would paste escape sequences

0.35.0
------
- Added `start` event that is triggered only once when tug finder starts.
  Since tug consumes the input stream asynchronously, the input list is not
  available unless you use `--sync`.
  ```sh
  seq 100 | tug --multi --sync --bind 'start:last+select-all+preview(echo welcome)'
  ```
- Added `--border-label` and `--border-label-pos` for putting label on the border
  ...
  tug --height=10 --border --border-label="╢ $label ╟" --color=label:italic:black

  # Left-aligned (positive integer)
  tug --height=10 --border --border-label="╢ $label ╟" --border-label-pos=3 --color=label:italic:black

  # Right-aligned (negative integer) on the bottom line (:bottom)
  tug --height=10 --border --border-label="╢ $label ╟" --border-label-pos=-3:bottom --color=label:italic:black
  ```
- Also added `--preview-label` and `--preview-label-pos` for the border of the
  preview window
  ```sh
  tug --preview 'cat {}' --border --preview-label=' Preview ' --preview-label-pos=2
  ```
- Info panel (match counter) will be followed by a horizontal separator by
  ...
  ```sh
  tug --separator=╸ --color=separator:green
  tug --separator=$(lolcat -f -F 1.4 <<< ▁▁▂▃▄▅▆▆▅▄▃▂▁▁) --info=inline
  ```
...
- Added support for adaptive `--height`. If the `--height` value is prefixed
  with `~`, tug will automatically determine the height in the range according
  to the input size.
  ```sh
  seq 1 | tug --height ~70% --border --padding 1 --margin 1
  seq 10 | tug --height ~70% --border --padding 1 --margin 1
  seq 100 | tug --height ~70% --border --padding 1 --margin 1
  ```
    - There are a few limitations
        - Not compatible with percent top/bottom margin/padding
          ```sh
          # This is not allowed (top/bottom margin in percent value)
          tug --height ~50% --border --margin 5%,10%

          # This is allowed (top/bottom margin in fixed value)
          tug --height ~50% --border --margin 2,10%
          ```
        - tug will not start until it can determine the right height for the input
          ```sh
          # tug will open immediately
          (sleep 2; seq 10) | tug --height 50%

          # tug will open after 2 seconds
          (sleep 2; seq 10) | tug --height ~50%
          (sleep 2; seq 1000) | tug --height ~50%
          ```
- Fixed tcell renderer used to render full-screen tug on Windows
...
- Added support for ANSI `strikethrough`
  ```sh
  printf "\e[9mdeleted" | tug --ansi
  tug --color fg+:strikethrough
  ```

0.32.1
------
- Fixed incorrect ordering of `--tiebreak=chunk`
- tug-tmux will show tug border instead of tmux popup border (requires tmux 3.3)
  ```sh
  tug-tmux -p70%
  tug-tmux -p70% --color=border:bright-red
  tug-tmux -p100%,60% --color=border:bright-yellow --border=horizontal --padding 1,5 --margin 1,0
  tug-tmux -p70%,100% --color=border:bright-green --border=vertical

  # Key bindings (CTRL-T, CTRL-R, ALT-C) will use these options
  export TUG_TMUX_OPTS='-p100%,60% --color=border:green --border=horizontal --padding 1,5 --margin 1,0'
  ```
...
- Added a new tiebreak `chunk`
    ...
    - Unlike the default `length`, this scheme works well with tabular input
      ...
      tug --height=6 --header-lines=2 --tiebreak=chunk --reverse --query=fo << "EOF"
...
- Added support for an alternative preview window layout that is activated
  ...
  tug --preview 'cat {}' --preview-window 'right,50%,border-left,<50(up,30%,border-bottom)'

  # Or you can just hide it like so
  tug --preview 'cat {}' --preview-window '<50(hidden)'
  ```
- tug now uses SGR mouse mode to properly support mouse on larger terminals
...
- Added `--ellipsis` option. You can take advantage of it to make tug
...
- Added `change-preview-window(...)` action
    - You can rotate through the different options separated by `|`
      ```sh
      tug --preview 'cat {}' --preview-window right:40% \
...
- tug on Windows will also use `$SHELL` to execute external programs
    - See #2638 and #2647
...
- 16 base ANSI colors can be specified by their names
  ```sh
  tug --color fg:3,fg+:11
  tug --color fg:yellow,fg+:bright-yellow
  ```
...
- Added `unbind` action. In the following Ripgrep launcher example, you can
  use `unbind(reload)` to switch to tug-only filtering mode.
    - See https://github.com/khulnasoft/tug/blob/master/ADVANCED.md#switching-to-tug-only-search-mode
- Vim plugin
    - Vim plugin will stop immediately even when the source command hasn't finished
      ```vim
      " tug will read the stream file while allowing other processes to append to it
      call tug#run({'source': 'cat /dev/null > /tmp/stream; tail -f /tmp/stream'})
      ```
...
- More border options for `--preview-window`
  ```sh
  tug --preview 'cat {}' --preview-window border-left
  tug --preview 'cat {}' --preview-window border-left --border horizontal
  tug --preview 'cat {}' --preview-window top:border-bottom
  tug --preview 'cat {}' --preview-window top:border-horizontal
  ```
- Automatically set `/dev/tty` as STDIN on execute action
  ```sh
  # Redirect /dev/tty to suppress "Vim: Warning: Input is not from a terminal"
  # ls | tug --bind "enter:execute(vim {} < /dev/tty)"

  # "< /dev/tty" part is no longer needed
  ls | tug --bind "enter:execute(vim {})"
  ```
- Bug fixes and improvements
- Signed and notarized macOS binaries
  (Huge thanks to [BACKERS.md](https://github.com/khulnasoft/khulnasoft/blob/main/BACKERS.md)!)
...
- Added support for fixed header in preview window
  ...
  tug --preview 'bat --style=header,grid --color=always {}' --preview-window '~3'
  ```
...
------
- Added `close` action
    - Close preview window if open, abort tug otherwise
- Bug fixes and improvements

0.25.0
------
- Text attributes set in `--color` are not reset when tug sees another
  `--color` option for the same element. This allows you to put custom text
  attributes in your `$TUG_DEFAULT_OPTS` and still have those attributes
  ...
  export TUG_DEFAULT_OPTS='--color fg+:italic,hl:-1:underline,hl+:-1:reverse:underline'

  tug

  # Different colors but you still have the attributes
  tug --color hl:176,hl+:177

  # Write "regular" if you want to clear the attributes
  tug --color hl:176:regular,hl+:177:regular
  ```
...
- You can assign a different color to the query string for when search is disabled
  ```sh
  tug --color query:#ffffff,disabled:#999999 --bind space:toggle-search
  ```
...
- Added `change-prompt` action
  ```sh
  tug --prompt 'foo> ' --bind $'a:change-prompt:\x1b[31mbar> '
  ```
- Bug fixes and improvements
...
- Added `--padding` option
  ```sh
  tug --margin 5% --padding 5% --border --preview 'cat {}' \
...
- Real-time rendering of preview window
  ```sh
  # tug can render preview window before the command completes
  tug --preview 'sleep 1; for i in $(seq 100); do echo $i; sleep 0.01; done'

  # Preview window can process ANSI escape sequence (CSI 2 J) for clearing the display
  tug --preview 'for i in $(seq 100000); do
...
- More `--border` options
  ...
  - Updated Vim plugin to use these new `--border` options
    ...
    let g:tug_layout = { 'window': { 'width': 1.0, 'height': 0.5, 'border': 'horizontal' } }

    " Popup with 100% height
    let g:tug_layout = { 'window': { 'width': 0.5, 'height': 1.0, 'border': 'vertical' } }

    " Similar to 'down' layout, but it uses a popup window and doesn't affect the window layout
    let g:tug_layout = { 'window': { 'width': 1.0, 'height': 0.5, 'yoffset': 1.0, 'border': 'top' } }

    " Opens on the right;
    "   'highlight' option is still supported but it will only take the foreground color of the group
    let g:tug_layout = { 'window': { 'width': 0.5, 'height': 1.0, 'xoffset': 1.0, 'border': 'left', 'highlight': 'Comment' } }
    ```
- To indicate if `--multi` mode is enabled, tug will print the number of
  ...
  seq 100 | tug --multi 5
    # 100/100 (0/5)
  ```
- Since 0.24.0, release binaries will be uploaded to https://github.com/khulnasoft/tug/releases
...
- Vim
    ...
    - Added `tug#exec()` function for getting the path of tug executable
...
- Added more options for `--bind`
    - `backward-eof` event
      ...
      tug --bind backward-eof:abort
      ```
    - `refresh-preview` action
      ```sh
      # Rerun preview command when you hit '?'
      tug --preview 'echo $RANDOM' --bind '?:refresh-preview'
      ```
    - `preview` action
      ```sh
      # Default preview command with an extra preview binding
      tug --preview 'file {}' --bind '?:preview:cat {}'

      # A preview binding with no default preview command
      # (Preview window is initially empty)
      tug --bind '?:preview:cat {}'

      # Preview window hidden by default, it appears when you first hit '?'
      tug --bind '?:preview:cat {}' --preview-window hidden
      ```
- Added preview window option for setting the initial scroll offset
  ...
  git grep --line-number '' |
    tug --delimiter : --preview 'nl {1}' --preview-window +{2}-5
  ```
...
- Vim plugin
    - `tmux` layout option for using tug-tmux
      ```vim
      let g:tug_layout = { 'tmux': '-p90%,60%' }
      ```
...
- tug-tmux
    - Supports tmux popup window (require tmux 3.2 or above)
        - ```sh
          ...
          # Window position
          tug-tmux -w 80% -h 40% -x 0 -y 0
          tug-tmux -w 80% -h 40% -y 1000

          # Write ordinary tug options after --
          tug-tmux -p -- --reverse --info=inline --margin 2,4 --border
          ```
...
- Fuzzy completion API changed
  ```sh
  # Previous: tug arguments given as a single string argument
  # - This style is still supported, but it's deprecated
  _tug_complete "--multi --reverse --prompt=\"doge> \"" "$@" < <(
    echo foo
  )

  # New API: multiple tug arguments before "--"
...
- Removed the immediate flicking of the screen on `reload` action.
  ```sh
  : | tug --bind 'change:reload:seq {q}' --phony
  ```
...
- It is now possible to split a composite bind action over multiple `--bind`
  ...
  ```sh
  tug --bind 'ctrl-a:up+up'

  # Can be now written as
  tug --bind 'ctrl-a:up' --bind 'ctrl-a:+up'
  ...
  tug --multi --bind 'ctrl-l:select-all+execute:less {+f}' --bind 'ctrl-l:+deselect-all'
  ```
- Fixed parse error of `--bind` expression where concatenated execute/reload
  action contains `+` character.
  ```sh
  tug --multi --bind 'ctrl-l:select-all+execute(less {+f})+deselect-all'
  ```
...
- Added `--phony` option which completely disables search functionality.
  Useful when you want to use tug only as a selector interface. See below.
- Added "reload" action for dynamically updating the input list without
  restarting tug. See https://github.com/khulnasoft/tug/issues/1750 to learn
  more about it.
  ```sh
  # Using tug as the selector interface for ripgrep
  RG_PREFIX="rg --column --line-number --no-heading --color=always --smart-case "
  INITIAL_QUERY="foo"
  TUG_DEFAULT_COMMAND="$RG_PREFIX '$INITIAL_QUERY' || true" \
    tug --bind "change:reload:$RG_PREFIX {q} || true" \
        --ansi --phony --query "$INITIAL_QUERY"
  ```
- `--multi` now takes an optional integer argument which indicates the maximum
  number of items that can be selected
  ```sh
  seq 100 | tug --multi 3 --reverse --height 50%
  ```
- If a placeholder expression for `--preview` and `execute` action (and the
  ...
  seq 100000 | tug --multi --bind ctrl-a:select-all \
...
- Due to the limitation of bash, fuzzy completion is enabled by default for
  ...
  ```sh
  # usage: _tug_setup_completion path|dir COMMANDS...
  _tug_setup_completion path git kubectl
  ```
...
- See https://github.com/khulnasoft/tug/milestone/15?closed=1 for more details
...
- Added `--no-unicode` option for drawing borders in non-Unicode, ASCII
  characters
- `TUG_PREVIEW_LINES` and `TUG_PREVIEW_COLUMNS` are exported to preview process
    - tug still overrides `LINES` and `COLUMNS` as before, but they may be
      reset by the default shell.
- Bug fixes and improvements
    - See https://github.com/khulnasoft/tug/milestone/14?closed=1
...
- Bug fixes and improvements
    - See https://github.com/khulnasoft/tug/milestone/13?closed=1
...
- tug can now start even when `/dev/tty` is not available by making an
  educated guess.
...
See https://github.com/khulnasoft/tug/milestone/12?closed=1 for the full list of
changes.
...
- Better error messages when the default command or `$TUG_DEFAULT_COMMAND`
  fails.
...
- tug now distinguishes mouse left click and right click (#1130)
...
- Added `accept-non-empty` action (#1162)
    - Same as accept, except that it prevents tug from exiting without any
      selection
...
A maintenance release for auxiliary scripts. tug binaries are not updated.

- Experimental support for the builtin terminal of Vim 8
    - tug can now run inside GVim
- Updated Vim plugin to better handle `&shell` issue on fish
- Fixed a bug of tug-tmux where invalid output is generated
- Fixed tug-tmux to work even when `tput` does not work
...
- New `change` event and `top` action for `--bind`
    ...
    - `tug --bind 'ctrl-w:unix-word-rubout+top,ctrl-u:unix-line-discard+top'`
...
- CTRL-Z (SIGSTOP) now works with tug
- tug will export `$TUG_PREVIEW_WINDOW` so that the scripts can use it
...
0.16.3
------
- Fixed a bug where tug incorrectly display the lines when straddling tab
...
- Placeholder expression used in `--preview` and `execute` action can
  optionally take `+` flag to be used with multiple selections
    - e.g. `git log --oneline | tug --multi --preview 'git show {+1}'`
- Added `execute-silent` action for executing a command silently without
  ...
  short-lived and you're not interested in its output.
    - e.g. `tug --bind 'ctrl-y:execute!(echo -n {} | pbcopy)'`
...
- Added support for composite actions in `--bind`. Multiple actions can be
  chained using `+` separator.
    - e.g. `tug --bind 'ctrl-y:execute(echo -n {} | pbcopy)+abort'`
- `--preview-window` with size 0 is allowed. This is used to make tug execute
...
- *Added `--height HEIGHT[%]` option*
    - tug can now display finder without occupying the full screen
...
- Can now be built with ncurses 6 or tcell to support extra features
    ...
    - See https://github.com/khulnasoft/tug/blob/master/BUILD.md
...
- `$SHELL` instead of `sh` for `execute` action and `$TUG_DEFAULT_COMMAND` (#481)
- Changes in fuzzy completion API
    - [`_tug_compgen_{path,dir}`](https://github.com/khulnasoft/tug/commit/9617647)
    - [`_tug_complete_COMMAND_post`](https://github.com/khulnasoft/tug/commit/8206746)
      for post-processing
...
- Added `toggle-in` and `toggle-out` actions
    ...
    - `export TUG_DEFAULT_OPTS="--bind tab:toggle-out,shift-tab:toggle-in"`
- Reduced the initial delay when `--tac` is not given
    - tug defers the initial rendering of the screen up to 100ms if the input
      ...
      impression that tug is not snappy enough. This commit reduces the
...
- Added `cancel` action which clears the input or closes the finder when the
  input is already empty
    - e.g. `export TUG_DEFAULT_OPTS="--bind esc:cancel"`
...
- More actions for `--bind`
    ...
    - `toggle-all`
    - `ignore`
- `execute(...)` action for running arbitrary command without leaving tug
    - `tug --bind "ctrl-m:execute(less {})"`
    - `tug --bind "ctrl-t:execute(tmux new-window -d 'vim {}')"`
...
- Added `--inline-info` option for saving screen estate (#202)
     ...
     - e.g. `let $TUG_DEFAULT_OPTS = $TUG_DEFAULT_OPTS.' --inline-info'`
...
- Fixed to print empty line if `--expect` is set and tug is completed by
...
If you provide a comma-separated list of keys with `--expect` option, tug will
allow you to select the match and complete the finder when any of the keys is
pressed. Additionally, tug will print the name of the key pressed as the first
line of the output so that your script can decide what to do next based on the
information.

```sh
tug --expect=ctrl-v,ctrl-t,alt-s,f1,f2,~,@
```
...
If you give `--ansi` option to tug, tug will interpret ANSI color codes from
...
By removing unnecessary copy of pointers, tug will use significantly smaller
amount of memory when it's started. The difference is hugely noticeable when
the input is extremely large. (e.g. `locate / | tug`)
...
When tug works in filtering mode (`--filter`) and sort is disabled
(`--no-sort`), there's no need to block until input is complete. The new
version of tug will print the matches on-the-fly when the following condition
is met:
...
However, in this case, tug processes the lines sequentially, so it cannot
utilize multiple cores, and tug will run slightly slower than the previous

...
// Sort criteria to use. Never changes once tug is started.
		...
		if (curr.color || curr.nth || curr.match) && idx > start {
			if curr.match {
				...
				if curr.color && theme.Colored {
					...
					// e.g. echo -e "\x1b[32;7mfoo\x1b[mbar" | tug --ansi --color bg+:1,hl+:-1:underline
					//      echo -e "\x1b[42mfoo\x1b[mbar" | tug --ansi --color bg+:1,hl+:-1:underline

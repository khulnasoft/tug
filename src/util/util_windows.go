...
func (x *Executor) Become(stdin *os.File, environ []string, command string) {
	...
	err := cmd.Start()
	if err != nil {
		fmt.Fprintf(os.Stderr, "tug (become): %s\n", err.Error())
		os.Exit(127)
	}
	err = cmd.Wait()
	if err != nil {
...
func (x *Executor) QuoteEntry(entry string) string {
	...
	case shellTypeCmd:
		/* Manually tested with the following commands:
		   tug --preview "echo {}"
		   tug --preview "type {}"
		   echo .git\refs\| tug --preview "dir {}"
		   echo .git\refs\\| tug --preview "dir {}"
		   echo .git\refs\\\| tug --preview "dir {}"
		   reg query HKCU | tug --reverse --bind "enter:reload(reg query {})"
		   tug --disabled --preview "echo {q} {n} {}" --query "&|<>()@^%!"
		   fd -H --no-ignore -td -d 4 | tug --preview "dir {}"
		   fd -H --no-ignore -td -d 4 | tug --preview "eza {}" --preview-window up
		   fd -H --no-ignore -td -d 4 | tug --preview "eza --color=always --tree --level=3 --icons=always {}"
		   fd -H --no-ignore -td -d 4 | tug --preview ".\eza.exe --color=always --tree --level=3 --icons=always {}" --with-shell "powershell -NoProfile -Command"
		*/

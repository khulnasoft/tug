...
import (
	...
	"github.com/khulnasoft-lab/go-shellwords"
	"github.com/rivo/uniseg"
)

const Usage = `tug is an interactive filter program for any kind of list.

It implements a "fuzzy" matching algorithm, so you can quickly type in patterns
with omitted characters and still get the results you want.

Project URL: https://github.com/khulnasoft/tug
Author: KhulnaSoft Lab <junegunn.c@gmail.com>

* See man page for more information: tug --man
...
Usage: tug [options]
  ...
  DISPLAY MODE
    --height=[~]HEIGHT[%]    Display tug window below the cursor with the given
                             height instead of using fullscreen.
                             A negative value is calculated as the terminal height
                             minus the given value.
                             If prefixed with '~', tug will determine the height
    ...
    --tmux[=OPTS]            Start tug in a tmux popup (requires tmux 3.3+)
  ...
  SCRIPTING
    ...
    --expect=KEYS            Comma-separated list of keys to complete tug
  ...
  DIRECTORY TRAVERSAL        (Only used when $TUG_DEFAULT_COMMAND is not set)
  ...
  ENVIRONMENT VARIABLES
    TUG_DEFAULT_COMMAND      Default command to use when input is tty
    TUG_DEFAULT_OPTS         Default options (e.g. '--layout=reverse --info=inline')
    TUG_DEFAULT_OPTS_FILE    Location of the file to read default options from
    TUG_API_KEY              X-API-Key header for HTTP server (--listen)
...
func parseOptions(index *int, opts *Options, allArgs []string) error {
	...
	for ; i < len(allArgs); i++ {
		...
		case "--no-tmux":
			opts.Tmux = nil
		case "--force-tty-in":
			// NOTE: We need this because `system('tug --tmux < /dev/tty')` doesn't
			// work on Neovim. Same as '-' option of tug-tmux.
...
// So we run it on tug.Run and not on ParseOptions.
...
func ParseOptions(useDefaults bool, args []string) (*Options, error) {
	...
	if useDefaults {
		// 1. Options from $TUG_DEFAULT_OPTS_FILE
		if path := os.Getenv("TUG_DEFAULT_OPTS_FILE"); path != "" {
			bytes, err := os.ReadFile(path)
			if err != nil {
				return nil, errors.New("$TUG_DEFAULT_OPTS_FILE: " + err.Error())
			}
		...
		// 2. Options from $TUG_DEFAULT_OPTS string
		words, parseErr := parseShellWords(os.Getenv("TUG_DEFAULT_OPTS"))
		if parseErr != nil {
			return nil, errors.New("$TUG_DEFAULT_OPTS: " + parseErr.Error())
		}
		if len(words) > 0 {
			if err := parseOptions(&index, opts, words); err != nil {
				return nil, errors.New("$TUG_DEFAULT_OPTS: " + err.Error())
			}
		}
	}
	...
	if len(opts.Scheme) == 0 {
		...
		if len(opts.Criteria) == 0 {
			// NOTE: Let's assume $TUG_DEFAULT_COMMAND generates a list of file paths.
			// But it is possible that it is set to a command that doesn't generate
			// file paths.
			//
			// In that case, you can either
			//   1. explicitly set --scheme=default,
			//   2. or replace $TUG_DEFAULT_COMMAND with an equivalent 'start:reload'

...
func TestReplacePlaceholder(t *testing.T) {
	...
	*/
	...
	templateToOutput[`{tug:query}`] = "{{.O}}" + query + "{{.O}}"
	templateToOutput[`{tug:action} {tug:prompt}`] = "backward-delete-char-eof 'prompt'"

	// IV. escaping placeholder
	templateToOutput[`\{}`] = `{}`
	templateToOutput[`\{q}`] = `{q}`
	templateToOutput[`\{tug:query}`] = `{tug:query}`
	templateToOutput[`\{tug:action}`] = `{tug:action}`
	templateToOutput[`\{++}`] = `{++}`
	templateToOutput[`{++}`] = templateToOutput[`{+}`]
...
// purpose of this test is to demonstrate some shortcomings of tug's templating system on Unix
...
// purpose of this test is to demonstrate some shortcomings of tug's templating system on Windows
func TestWindowsCommands(t *testing.T) {
	...
	tests := []testCase{
		...
		// notepad++'s parser can't handle `-n"12"` generate by tug, expects `-n12`
		...
		{give{`cmd /c {f}`, ``, newItems(`cat "C:\test.txt"`)}, want{match: `^cmd /c .*\tug-preview-[0-9]{9}$`}},
	}
	testCommands(t, tests)
}

// purpose of this test is to demonstrate some shortcomings of tug's templating system on Windows in Powershell
func TestPowershellCommands(t *testing.T) {
	...
	tests := []testCase{
		...
		*/
		...
		// to explorer, which will prompt user to pick editing program for the tug-preview file
		// the temp file contains: `cat "C:\test.txt"`
		// TODO this should actually work
		{give{`powershell -NoProfile -Command {f}`, ``, newItems(`cat "C:\test.txt"`)}, want{match: `^powershell -NoProfile -Command .*\tug-preview-[0-9]{9}$`}},
	}

	// to force powershell-style escaping we temporarily set environment variable that tug honors
...
// Item represents one line in tug UI. Usually it is relative path to files and folders.

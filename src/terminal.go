...
/*
Placeholder regex is used to extract placeholders from tug's template
...
func init() {
	placeholder = regexp.MustCompile(`\\?(?:{[+sf]*[0-9,-.]*}|{q(?::s?[0-9,-.]+)?}|{tug:(?:query|action|prompt)}|{\+?f?nf?})`)
...
func NewTerminal(opts *Options, eventBox *util.EventBox, executor *util.Executor) (*Terminal, error) {
	...
	// when you run tug multiple times in your Go program. Closing it is known to
	...
	if fullscreen {
		...
		maxHeightFunc := func(termHeight int) int {
			// Minimum height required to render tug excluding margin and padding
...
func (t *Terminal) environImpl(forPreview bool) []string {
	env := os.Environ()
	if t.listenPort != nil {
		env = append(env, fmt.Sprintf("TUG_PORT=%d", *t.listenPort))
	}
	env = append(env, "TUG_QUERY="+string(t.input))
	env = append(env, "TUG_ACTION="+t.lastAction.Name())
	env = append(env, "TUG_KEY="+t.lastKey)
	env = append(env, "TUG_PROMPT="+string(t.promptString))
	env = append(env, "TUG_PREVIEW_LABEL="+t.previewLabelOpts.label)
	env = append(env, "TUG_BORDER_LABEL="+t.borderLabelOpts.label)
	env = append(env, "TUG_LIST_LABEL="+t.listLabelOpts.label)
	if len(t.nthCurrent) > 0 {
		env = append(env, "TUG_NTH="+RangesToString(t.nthCurrent))
	}
	inputState := "enabled"
	if t.inputless {
		inputState = "hidden"
	...
	env = append(env, "TUG_INPUT_STATE="+inputState)
	env = append(env, fmt.Sprintf("TUG_TOTAL_COUNT=%d", t.count))
	env = append(env, fmt.Sprintf("TUG_MATCH_COUNT=%d", t.merger.Length()))
	env = append(env, fmt.Sprintf("TUG_SELECT_COUNT=%d", len(t.selected)))
	env = append(env, fmt.Sprintf("TUG_LINES=%d", t.areaLines))
	env = append(env, fmt.Sprintf("TUG_COLUMNS=%d", t.areaColumns))
	env = append(env, fmt.Sprintf("TUG_POS=%d", util.Min(t.merger.Length(), t.cy+1)))
	env = append(env, fmt.Sprintf("TUG_CLICK_HEADER_LINE=%d", t.clickHeaderLine))
	env = append(env, fmt.Sprintf("TUG_CLICK_HEADER_COLUMN=%d", t.clickHeaderColumn))
	...
	if pwindowSize.Lines > 0 {
		...
		env = append(env, "TUG_PREVIEW_"+lines)
		env = append(env, "TUG_PREVIEW_"+columns)
		env = append(env, fmt.Sprintf("TUG_PREVIEW_TOP=%d", t.tui.Top()+t.pwindow.Top()))
		env = append(env, fmt.Sprintf("TUG_PREVIEW_LEFT=%d", t.pwindow.Left()))
	}
...
// Extra number of lines needed to display tug
...
func (t *Terminal) resizeWindows(forcePreview bool, redrawBorder bool) {
	...
	if forcePreview || t.needPreviewWindow() {
		...
		resizePreviewWindows = func(previewOpts *previewOpts) {
			...
			case posLeft, posRight:
				...
				if previewOpts.position == posLeft {
					...
					//   tug --bind 'space:preview(seq 100)' --preview-window left,1
					if !hasListBorder {
					...
					//  tug --preview 'seq 500' --preview-window border-left --border
					//  tug --preview 'seq 500' --preview-window border-left --border --list-border
					//  tug --preview 'seq 500' --preview-window border-left --border --input-border
		...
		if t.borderShape.HasRight() && !listStickToRight {
			...
			// tug --preview 'seq 1000' --preview-window border-left --bind space:change-preview-window:border-rounded --border vertical
			// tug --preview 'seq 1000' --preview-window up,hidden --bind space:toggle-preview --border vertical
			y := 0
			if t.borderShape.HasTop() {
				y++
			}
	...
	if hasInputWindow {
		...
		if shift > 0 && !t.inputBorderShape.Visible() {
			...
			// e.g.
			//  tug --list-border --header-border --bind 'space:change-header(hello),enter:change-header()'
...
func (t *Terminal) truncateQuery() {
	// We're limiting the length of the query not to make tug unresponsive when
...
func (t *Terminal) printHeaderImpl(window tui.Window, borderShape tui.BorderShape, lines1 []string, lines2 []string) {
	...
	//   tug --header-lines 3 --style full --no-list-border
	//   tug --header-lines 3 --style full --no-header-border
	//   tug --header-lines 3 --style full --no-header-border --no-input-border
	indentSize := t.pointerLen + t.markerLen
	if window != nil {
...
func parsePlaceholder(match string) (bool, string, placeholderFlags) {
	...
	if strings.HasPrefix(match, "{tug:") {
		// {tug:*} are not determined by the current item
...
func replacePlaceholder(params replacePlaceholderParams) (string, []string) {
	...
	replaced := placeholder.ReplaceAllStringFunc(params.template, func(match string) string {
		...
		case match == "{tug:action}":
			return params.lastAction.Name()
		case match == "{tug:prompt}":
...
func (t *Terminal) executeCommand(template string, forcePlus bool, background bool, capture bool, firstLineOnly bool, info string) string {
	...
	cmd.Env = t.environ()
	if len(info) > 0 {
		cmd.Env = append(cmd.Env, "TUG_INFO="+info)
	}
...
func (t *Terminal) addClickHeaderWord(env []string) []string {
	/*
	 * echo $'HL1\nHL2' | tug --header-lines 3 --header $'H1\nH2' --header-lines-border --bind 'click-header:preview:env | grep TUG_CLICK'
	 *
	...
	for idx, token := range words {
		...
		if colNum >= prefixWidth && colNum < prefixWidth+trimWidth {
			env = append(env, fmt.Sprintf("TUG_CLICK_HEADER_WORD=%s", trimmed))
			nth := fmt.Sprintf("TUG_CLICK_HEADER_NTH=%d", idx+1)
			if idx == len(words)-1 {
				nth += ".."
			}
			env = append(env, nth)
		...
		go func() {
			for {
				...
				case <-ctx.Done():
					return
				case s := <-intChan:
					// Don't quit by SIGINT while executing because it should be for the executing command and not for tug itself
		...
		Action:
			...
			case actChangePreviewWindow:
				...
				case previewOptsDifferentLayout:
					...
					// tug --bind space:preview:ls --bind 'enter:change-preview-window:down|left|up|hidden|'
			...
			add := func(i int) bool {
				...
				if linesSum >= numItems {
					/*
						...
						printf "file1\0file2\0file3\0" | tug --height=5 --read0 --bind load:last --reverse

						# Should not truncate the last item
						printf "file\n1\0file\n2\0file\n3\0" | tug --height=5 --read0 --bind load:last --reverse
					*/

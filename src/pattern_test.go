...
func TestOrigTextAndTransformed(t *testing.T) {
	...
	tokens := Tokenize("khulnasoft", Delimiter{})
	trans := Transform(tokens, []Range{{1, 1}})

	origBytes := []byte("khulnasoft.choi")
	for _, extended := range []bool{false, true} {
		chunk := Chunk{count: 1}
		chunk.items[0] = Item{
			text:        util.ToChars([]byte("khulnasoft")),
			origText:    &origBytes,
			transformed: &transformed{pattern.revision, trans}}
		pattern.extended = extended
		matches := pattern.matchChunk(&chunk, nil, slab) // No cache
		if !(matches[0].item.text.ToString() == "khulnasoft" &&
			string(*matches[0].item.origText) == "khulnasoft.choi" &&
		...
		if !(match.item.text.ToString() == "khulnasoft" &&
			string(*match.item.origText) == "khulnasoft.choi" &&

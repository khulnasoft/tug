...
func (r *LightRenderer) Init() error {
	...
	if r.fullscreen {
		...
		// We assume that --no-clear is used for repetitive relaunching of tug.
		...
		// time frame between tug processes where the user keystrokes are not
		// captured by either of tug process which can cause x offset to be
...
func (r *LightRenderer) NewWindow(top int, left int, width int, height int, windowType WindowType, borderStyle BorderStyle, erase bool) Window {
	...
	if erase && !w.bg.IsDefault() && w.border.shape != BorderNone {
		// tug --color bg:blue --border --padding 1,2
		w.Erase()
	}
	w.drawBorder(false)
	return w
...
func (w *LightWindow) setBg() string {
	...
	// e.g. printf "foo\rbar" | tug --ansi --preview 'printf "foo\rbar"'
	return "\x1b[m"
}

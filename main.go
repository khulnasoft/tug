...
import (
	...
	tug "github.com/khulnasoft/tug/src"
	"github.com/khulnasoft/tug/src/protector"
)
...
func main() {
	...
	options, err := tug.ParseOptions(true, os.Args[1:])
	if err != nil {
		exit(tug.ExitError, err)
		return
	}
	if options.Bash {
	...
	if options.Man {
		file := tug.WriteTemporaryFile([]string{string(manPage)}, "\n")
		if len(file) == 0 {

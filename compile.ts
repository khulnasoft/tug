...
const setDeveloperState = (key: string, value: string | null) => {
  try {
    return execSync(
      `tug _ local-state plugins.developer.${key} ${value === null ? "--delete" : value}`
    );
  } catch (_) {
    return null;
  }
...
async function runProgram(options: { watch?: boolean }) {
  ...
  if (options.watch) {
    ...
    if (isMacOS) {
      const globalTugAppPath = path.join("/", "Applications", "Tug.app");
      const localTugAppPath = path.join(os.homedir(), "Applications", "Tug.app");

      if (!fs.existsSync(globalTugAppPath) && !fs.existsSync(localTugAppPath)) {
        warn("Tug is not installed", chalk.bold(chalk.cyan("Download Tug at:")), "https://khulnasoft.com/");
      } else {
        try {
          execSync("tug --version");
        } catch (e) {
          warn(
            "Tug cli is not installed",
            chalk.bold(
              chalk.cyan(
    ...
    console.log(
      ...
      `3. Test your changes ${chalk.bold("instantly")} in tug\n`,

...
It tugures out the version being used by taking the output of the `rbenv version-name` command.
...
The `PYENV_VERSION` environment variable will be used if specified. Otherwise it tugures out the version being used by taking the output of the `pyenv version-name` command.

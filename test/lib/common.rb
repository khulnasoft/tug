...
UNSETS = %w[
  TUG_DEFAULT_COMMAND TUG_DEFAULT_OPTS
  TUG_TMUX TUG_TMUX_OPTS
  TUG_CTRL_T_COMMAND TUG_CTRL_T_OPTS
  TUG_ALT_C_COMMAND
  TUG_ALT_C_OPTS TUG_CTRL_R_OPTS
  TUG_API_KEY
].freeze
DEFAULT_TIMEOUT = 10
...
Dir.chdir(BASE)
TUG = "TUG_DEFAULT_OPTS=\"--no-scrollbar --pointer \\> --marker \\>\" TUG_DEFAULT_COMMAND= #{BASE}/bin/tug".freeze
...
class Shell
  class << self
    ...
    def fish
      "unset #{UNSETS.join(' ')}; rm -f ~/.local/share/fish/tug_test_history; TUG_DEFAULT_OPTS=\"--no-scrollbar --pointer '>' --marker '>'\" fish_history=tug_test fish"
    end
  end
end
...
class TestBase < Minitest::Test
  TEMPNAME = Dir::Tmpname.create(%w[tug]) {}
  FIFONAME = Dir::Tmpname.create(%w[tug-fifo]) {}
  ...
  def tug_output_lines
    tug_output.lines(chomp: true)
  end
  ...
  # Run tug with its output piped to a fifo
  def tug(*opts)
    raise 'tug_output not taken' if @thread

    @thread = Thread.new { File.read(FIFONAME) }
    tug!(*opts) + " > #{FIFONAME.shellescape}"
  end

...
class TestServer < TestInteractive
  def test_listen
    { '--listen 6266' => -> { URI('http://localhost:6266') },
      "--listen --sync --bind 'start:execute-silent:echo $TUG_PORT > /tmp/tug-port'" =>
        -> { URI("http://localhost:#{File.read('/tmp/tug-port').chomp}") } }.each do |opts, fn|
      tmux.send_keys "seq 10 | tug #{opts}", :Enter
  ...
  def test_listen_with_api_key
    ...
    tmux.send_keys 'seq 10 | TUG_API_KEY=123abc tug --listen 6266', :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
    # Incorrect API Key

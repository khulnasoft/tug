...
# Testing basic features of tug
class TestCore < TestInteractive
  def test_tug_default_command
    tmux.send_keys tug.sub('TUG_DEFAULT_COMMAND=', "TUG_DEFAULT_COMMAND='echo hello'"), :Enter
    tmux.until { |lines| assert_equal '> hello', lines[-3] }

    tmux.send_keys :Enter
    assert_equal 'hello', tug_output
  end

  def test_tug_default_command_failure
    tmux.send_keys tug.sub('TUG_DEFAULT_COMMAND=', 'TUG_DEFAULT_COMMAND=false'), :Enter
    tmux.until { |lines| assert_includes lines[-2], '  [Command failed: false] ─' }
    tmux.send_keys :Enter
  end

  def test_key_bindings
    tmux.send_keys "#{TUG} -q 'foo bar foo-bar'", :Enter
  ...
  def test_file_word
    tmux.send_keys "#{TUG} -q '--/foo bar/foo-bar/baz' --filepath-word", :Enter
  ...
  def test_multi_order
    tmux.send_keys "seq 1 10 | #{tug(:multi)}", :Enter
    tmux.until { |lines| assert_equal '>', lines.last }
    ...
    tmux.send_keys 'C-M'
    assert_equal %w[3 2 5 6 8 7], tug_output_lines
  end

  def test_multi_max
    tmux.send_keys "seq 1 10 | #{TUG} -m 3 --bind A:select-all,T:toggle-all --preview 'echo [{+}]/{}'", :Enter
  ...
  def test_multi_action
    tmux.send_keys "seq 10 | #{TUG} --bind 'a:change-multi,b:change-multi(3),c:change-multi(xxx),d:change-multi(0)'", :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
  ...
  def test_with_nth
    [true, false].each do |multi|
      tmux.send_keys "(echo '  1st 2nd 3rd/';
                       ...
                       #{tug(multi && :multi, :x, :nth, 2, :with_nth, '2,-1,1')}",
                     :Enter
      ...
      if multi
        ...
        tmux.send_keys :Enter
        assert_equal ['  1st 2nd 3rd/', '  first second third/'], tug_output_lines
      else
        tmux.send_keys '^', '3'
        tmux.until { |lines| assert_equal '  1/2', lines[-2] }
        tmux.send_keys :Enter
        assert_equal ['  1st 2nd 3rd/'], tug_output_lines
      end
    end
  end
  ...
  def test_scroll
    [true, false].each do |rev|
      tmux.send_keys "seq 1 100 | #{tug(rev && :reverse)}", :Enter
      ...
      tmux.send_keys :Enter
      assert_equal '100', tug_output
    end
  end

  def test_select_1
    tmux.send_keys "seq 1 100 | #{tug(:with_nth, '..,..', :print_query, :q, 5555, :'1')}", :Enter
    assert_equal %w[5555 55], tug_output_lines
  end

  def test_select_1_accept_nth
    tmux.send_keys "seq 1 100 | #{tug(:with_nth, '..,..', :print_query, :q, 5555, :'1', :accept_nth, '"{1} // {1}"')}", :Enter
    assert_equal ['5555', '55 // 55'], tug_output_lines
  end

  def test_exit_0
    tmux.send_keys "seq 1 100 | #{tug(:with_nth, '..,..', :print_query, :q, 555_555, :'0')}", :Enter
    assert_equal %w[555555], tug_output_lines
  end

  def test_select_1_exit_0_fail
    [:'0', :'1', %i[1 0]].each do |opt|
      tmux.send_keys "seq 1 100 | #{tug(:print_query, :multi, :q, 5, *opt)}", :Enter
      tmux.until { |lines| assert_equal '> 5', lines.last }
      tmux.send_keys :BTab, :BTab, :BTab
      tmux.until { |lines| assert_equal '  19/100 (3)', lines[-2] }
      tmux.send_keys :Enter
      assert_equal %w[5 5 50 51], tug_output_lines
    end
  end

  def test_query_unicode
    tmux.paste "(echo abc; echo $'\\352\\260\\200\\353\\202\\230\\353\\213\\244') | #{tug(:query, "$'\\352\\260\\200\\353\\213\\244'")}"
    tmux.until { |lines| assert_equal '  1/2', lines[-2] }
    tmux.send_keys :Enter
    assert_equal %w[가나다], tug_output_lines
  end

  def test_sync
    tmux.send_keys "seq 1 100 | #{TUG} --multi | awk '{print $1 $1}' | #{tug(:sync)}", :Enter
    ...
    assert_equal %w[9090], tug_output_lines
  end

  def test_tac
    tmux.send_keys "seq 1 1000 | #{tug(:tac, :multi)}", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys :BTab, :BTab, :BTab
    tmux.until { |lines| assert_equal '  1000/1000 (3)', lines[-2] }
    tmux.send_keys :Enter
    assert_equal %w[1000 999 998], tug_output_lines
  end
  ...
  def test_tac_sort
    ...
    tmux.send_keys :Enter
    assert_equal %w[99 999 998], tug_output_lines
  end

  def test_tac_nosort
    tmux.send_keys "seq 1 1000 | #{tug(:tac, :no_sort, :multi)}", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys '00'
    ...
    tmux.send_keys :Enter
    assert_equal %w[1000 900 800], tug_output_lines
  end

  def test_expect
    test = lambda do |key, feed, expected = key|
      tmux.send_keys "seq 1 100 | #{tug(:expect, key, :prompt, "[#{key}]")}", :Enter
      tmux.until { |lines| assert_equal '  100/100', lines[-2] }
      tmux.send_keys '55'
      tmux.until { |lines| assert_equal '  1/100', lines[-2] }
      tmux.send_keys(*feed)
      tmux.prepare
      assert_equal [expected, '55'], tug_output_lines
    end
  ...
  def test_expect_with_bound_actions
    tmux.send_keys "seq 1 100 | #{tug('--query 1 --print-query --expect z --bind z:up+up')}", :Enter
    tmux.until { |lines| assert_equal 20, lines.match_count }
    tmux.send_keys('z')
    assert_equal %w[1 z 1], tug_output_lines
  end

  def test_expect_print_query
    tmux.send_keys "seq 1 100 | #{tug('--expect=alt-z', :print_query)}", :Enter
    tmux.until { |lines| assert_equal '  100/100', lines[-2] }
    tmux.send_keys '55'
    tmux.until { |lines| assert_equal '  1/100', lines[-2] }
    tmux.send_keys :Escape, :z
    assert_equal %w[55 alt-z 55], tug_output_lines
  end

  def test_expect_printable_character_print_query
    tmux.send_keys "seq 1 100 | #{tug('--expect=z --print-query')}", :Enter
    tmux.until { |lines| assert_equal '  100/100', lines[-2] }
    tmux.send_keys '55'
    tmux.until { |lines| assert_equal '  1/100', lines[-2] }
    tmux.send_keys 'z'
    assert_equal %w[55 z 55], tug_output_lines
  end

  def test_expect_print_query_select_1
    tmux.send_keys "seq 1 100 | #{tug('-q55 -1 --expect=alt-z --print-query')}", :Enter
    assert_equal ['55', '', '55'], tug_output_lines
  end

  def test_toggle_sort
    ['--toggle-sort=ctrl-r', '--bind=ctrl-r:toggle-sort'].each do |opt|
      tmux.send_keys "seq 1 111 | #{tug("-m +s --tac #{opt} -q11")}", :Enter
      ...
      tmux.send_keys :Enter
      assert_equal %w[111 11], tug_output_lines
    end
  end

  def test_invalid_cache
    tmux.send_keys "(echo d; echo D; echo x) | #{tug('-q d')}", :Enter
  ...
  def test_invalid_cache_query_type
    command = %[(echo 'foo$bar'; echo 'barfoo'; echo 'foo^bar'; echo "foo'1-2"; seq 100) | #{TUG}]
  ...
  def test_bind
    tmux.send_keys "seq 1 1000 | #{tug('-m --bind=ctrl-j:accept,u,:,U:up,X,,,Z:toggle-up,t:toggle')}", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys 'uU:', 'X,Z', 'tt', 'uu', 'ttt', 'C-j'
    assert_equal %w[4 5 6 9], tug_output_lines
  end

  def test_bind_print_query
    tmux.send_keys "seq 1 1000 | #{tug('-m --bind=ctrl-j:print-query')}", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys 'print-my-query', 'C-j'
    assert_equal %w[print-my-query], tug_output_lines
  end

  def test_bind_replace_query
    tmux.send_keys "seq 1 1000 | #{tug('--print-query --bind=ctrl-j:replace-query')}", :Enter
    tmux.send_keys '1'
  ...
  def test_select_all_deselect_all_toggle_all
    tmux.send_keys "seq 100 | #{tug('--bind ctrl-a:select-all,ctrl-d:deselect-all,ctrl-t:toggle-all --multi')}", :Enter
  ...
  def test_history
    history_file = '/tmp/tug-test-history'
    ...
    input.each do |keys|
      tmux.prepare
      tmux.send_keys "seq 100 | #{TUG} #{opts}", :Enter
      tmux.until { |lines| assert_equal '  100/100', lines[-2] }
      tmux.send_keys keys
      tmux.until { |lines| assert_equal '  1/100', lines[-2] }
      tmux.send_keys :Enter
    ...
    # Respect --bind option
    tmux.send_keys "seq 100 | #{TUG} #{opts} --bind ctrl-p:next-history,ctrl-n:previous-history", :Enter
    tmux.until { |lines| assert_equal '  100/100', lines[-2] }
  ...
  def test_cycle
    tmux.send_keys "seq 8 | #{TUG} --cycle", :Enter
  ...
  def test_header_lines
    tmux.send_keys "seq 100 | #{tug('--header-lines=10 -q 5')}", :Enter
    2.times do
      tmux.until do |lines|
    ...
    tmux.send_keys :Enter
    assert_equal '50', tug_output
  end

  def test_header_lines_reverse
    tmux.send_keys "seq 100 | #{tug('--header-lines=10 -q 5 --reverse')}", :Enter
    2.times do
      tmux.until do |lines|
  ...
  def test_header_lines_reverse_list
    tmux.send_keys "seq 100 | #{tug('--header-lines=10 -q 5 --layout=reverse-list')}", :Enter
    2.times do
      tmux.until do |lines|
  ...
  def test_header_lines_overflow
    tmux.send_keys "seq 100 | #{tug('--header-lines=200')}", :Enter
    tmux.until do |lines|
      assert_equal '  0/0', lines[-2]
      assert_equal '  1', lines[-3]
    end
    tmux.send_keys :Enter
    assert_equal '', tug_output
  end

  def test_header_lines_with_nth
    tmux.send_keys "seq 100 | #{tug('--header-lines 5 --with-nth 1,1,1,1,1')}", :Enter
    tmux.until do |lines|
    ...
    tmux.send_keys :Enter
    assert_equal '6', tug_output
  end

  def test_header
    tmux.send_keys %[seq 100 | #{TUG} --header "$(head -5 #{FILE})"], :Enter
  ...
  def test_header_reverse
    tmux.send_keys %[seq 100 | #{TUG} --header "$(head -5 #{FILE})" --reverse], :Enter
    header = File.readlines(FILE, chomp: true).take(5)
    tmux.until do |lines|
  ...
  def test_header_reverse_list
    tmux.send_keys %[seq 100 | #{TUG} --header "$(head -5 #{FILE})" --layout=reverse-list], :Enter
    header = File.readlines(FILE, chomp: true).take(5)
    tmux.until do |lines|
      assert_equal '  100/100', lines[-2]
      assert_equal header.map { |line| "  #{line}".rstrip }, lines[-7..-3]
  ...
  def test_header_and_header_lines
    tmux.send_keys %[seq 100 | #{TUG} --header-lines 10 --header "$(head -5 #{FILE})"], :Enter
    header = File.readlines(FILE, chomp: true).take(5)
    tmux.until do |lines|
  ...
  def test_header_and_header_lines_reverse
    tmux.send_keys %[seq 100 | #{TUG} --reverse --header-lines 10 --header "$(head -5 #{FILE})"], :Enter
    header = File.readlines(FILE, chomp: true).take(5)
    tmux.until do |lines|
  ...
  def test_header_and_header_lines_reverse_list
    tmux.send_keys %[seq 100 | #{TUG} --layout=reverse-list --header-lines 10 --header "$(head -5 #{FILE})"], :Enter
    header = File.readlines(FILE, chomp: true).take(5)
    tmux.until do |lines|
      assert_equal '  90/90', lines[-2]
      assert_equal header.map { |line| "  #{line}".rstrip }, lines[-7...-2]
  ...
  def test_cancel
    tmux.send_keys "seq 10 | #{TUG} --bind 2:cancel", :Enter
  ...
  def test_margin
    tmux.send_keys "yes | head -1000 | #{TUG} --margin 5,3", :Enter
    tmux.until do |lines|
  ...
  def test_margin_reverse
    tmux.send_keys "seq 1000 | #{TUG} --margin 7,5 --reverse", :Enter
    tmux.until { |lines| assert_equal '       1000/1000', lines[1 + 7] }
    tmux.send_keys :Enter
  end

  def test_margin_reverse_list
    tmux.send_keys "yes | head -1000 | #{TUG} --margin 5,3 --layout=reverse-list", :Enter
    tmux.until do |lines|
      assert_equal '', lines[4]
      assert_equal '   > y', lines[5]
    end
  ...
  def test_tabstop
    writelines(%W[f\too\tba\tr\tbaz\tbarfooq\tux])
    {
      ...
      tmux.prepare
      tmux.send_keys %(cat #{tempname} | tug --tabstop=#{ts}), :Enter
  ...
  def test_exit_0_exit_code
    `echo foo | #{TUG} -q bar -0`
    assert_equal 1, $CHILD_STATUS.exitstatus
  end

  def test_invalid_option
    lines = `#{TUG} --foobar 2>&1`
  ...
  def test_exitstatus_empty
    { '99' => '0', '999' => '1' }.each do |query, status|
      tmux.send_keys "seq 100 | #{TUG} -q #{query}; echo --$?--", :Enter
  ...
  def test_hscroll_off
    ...
    [0, 3, 6].each do |off|
      tmux.prepare
      tmux.send_keys "#{TUG} --hscroll-off=#{off} -q 0 --bind space:toggle-hscroll < #{tempname}", :Enter
  ...
  def test_partial_caching
    tmux.send_keys 'seq 1000 | tug -e', :Enter
  ...
  def test_jump
    tmux.send_keys "seq 1000 | #{tug("--multi --jump-labels 12345 --bind 'ctrl-j:jump'")}", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys 'C-j'
    ...
    tmux.until { |lines| assert_equal '>>1', lines[-3] }
    tmux.send_keys :Enter
    assert_equal %w[5 2 1], tug_output_lines
  end

  def test_jump_accept
    tmux.send_keys "seq 1000 | #{tug("--multi --jump-labels 12345 --bind 'ctrl-j:jump-accept'")}", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys 'C-j'
    tmux.until { |lines| assert_equal '5 5', lines[-7] }
    tmux.send_keys '3'
    assert_equal '3', tug_output
  end

  def test_jump_events
    tmux.send_keys "seq 1000 | #{TUG} --multi --jump-labels 12345 --bind 'ctrl-j:jump,jump:preview(echo jumped to {}),jump-cancel:preview(echo jump cancelled at {})'", :Enter
    tmux.until { |lines| assert_equal '  1000/1000 (0)', lines[-2] }
    tmux.send_keys 'C-j'
  ...
  def test_jump_no_pointer
    tmux.send_keys "seq 100 | #{TUG} --pointer= --jump-labels 12345 --bind ctrl-j:jump", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.send_keys 'C-j'
    tmux.until { |lines| assert_equal '5 5', lines[-7] }
    tmux.send_keys 'C-c'
  ...
  def test_jump_no_pointer_no_marker
    tmux.send_keys "seq 100 | #{TUG} --pointer= --marker= --jump-labels 12345 --bind ctrl-j:jump", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.send_keys 'C-j'
  ...
  def test_pointer
    tmux.send_keys "seq 10 | #{tug("--pointer '>>'")}", :Enter
    # Assert that specified pointer is displayed
    tmux.until { |lines| assert_equal '>> 1', lines[-3] }
  end

  def test_pointer_with_jump
    tmux.send_keys "seq 10 | #{TUG} --multi --jump-labels 12345 --bind 'ctrl-j:jump' --pointer '>>'", :Enter
  ...
  def test_marker
    tmux.send_keys "seq 10 | #{TUG} --multi --marker '>>'", :Enter
    tmux.until { |lines| assert_equal '  10/10 (0)', lines[-2] }
    tmux.send_keys :BTab
  ...
  def test_no_clear
    tmux.send_keys "seq 10 | #{tug('--no-clear --inline-info --height 5')}", :Enter
    prompt = '>   < 10/10'
    tmux.until { |lines| assert_equal prompt, lines[-1] }
    tmux.send_keys :Enter
    assert_equal %w[1], tug_output_lines
    tmux.until { |lines| assert_equal prompt, lines[-1] }
  end

  def test_info_hidden
    tmux.send_keys 'seq 10 | tug --info=hidden --no-separator', :Enter
    tmux.until { |lines| assert_equal '> 1', lines[-2] }
  end

  def test_info_inline_separator
    tmux.send_keys 'seq 10 | tug --info=inline:___ --no-separator', :Enter
    tmux.until { |lines| assert_equal '>  ___10/10', lines[-1] }
  end

  def test_change_first_last
    tmux.send_keys %(seq 1000 | #{TUG} --bind change:first,alt-Z:last), :Enter
  ...
  def test_pos
    tmux.send_keys %(seq 1000 | #{TUG} --bind 'a:pos(3),b:pos(-3),c:pos(1),d:pos(-1),e:pos(0)' --preview 'echo {}/{}'), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys :a
  ...
  def test_put
    tmux.send_keys %(seq 1000 | #{TUG} --bind 'a:put+put,b:put+put(ravo)' --preview 'echo {q}/{q}'), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys :a
  ...
  def test_accept_non_empty
    tmux.send_keys %(seq 1000 | #{tug('--print-query --bind enter:accept-non-empty')}), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys 'foo'
    tmux.until { |lines| assert_equal '  0/1000', lines[-2] }
    # tug doesn't exit since there's no selection
    tmux.send_keys :Enter
    tmux.until { |lines| assert_equal '  0/1000', lines[-2] }
    tmux.send_keys 'C-u'
    tmux.until { |lines| assert_equal '  1000/1000', lines[-2] }
    ...
    tmux.send_keys :Enter
    assert_equal %w[999 999], tug_output_lines
  end

  def test_accept_non_empty_with_multi_selection
    tmux.send_keys %(seq 1000 | #{tug('-m --print-query --bind enter:accept-non-empty')}), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys :Tab
    tmux.until { |lines| assert_equal '  1000/1000 (1)', lines[-2] }
    tmux.send_keys 'foo'
    tmux.until { |lines| assert_equal '  0/1000 (1)', lines[-2] }
    # tug will exit in this case even though there's no match for the current query
    tmux.send_keys :Enter
    assert_equal %w[foo 1], tug_output_lines
  end

  def test_accept_non_empty_with_empty_list
    tmux.send_keys %(: | #{tug('-q foo --print-query --bind enter:accept-non-empty')}), :Enter
    tmux.until { |lines| assert_equal '  0/0', lines[-2] }
    tmux.send_keys :Enter
    # tug will exit anyway since input list is empty
    assert_equal %w[foo], tug_output_lines
  end

  def test_accept_or_print_query_without_match
    tmux.send_keys %(seq 1000 | #{tug('--bind enter:accept-or-print-query')}), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys 99_999
    tmux.until { |lines| assert_equal 0, lines.match_count }
    tmux.send_keys :Enter
    assert_equal %w[99999], tug_output_lines
  end
  ...
  def test_accept_or_print_query_with_match
    ...
    tmux.until { |lines| assert_equal 1, lines.match_count }
    tmux.send_keys :Enter
    assert_equal %w[99], tug_output_lines
  end

  def test_accept_or_print_query_with_multi_selection
    tmux.send_keys %(seq 1000 | #{tug('--bind enter:accept-or-print-query --multi')}), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys :BTab, :BTab, :BTab
    ...
    tmux.until { |lines| assert_equal 0, lines.match_count }
    tmux.send_keys :Enter
    assert_equal %w[1 2 3], tug_output_lines
  end

  def test_inverse_only_search_should_not_sort_the_result
    # Filter
    assert_equal %w[aaaaa b ccc],
                 `printf '%s\n' aaaaa b ccc BAD | #{TUG} -f '!bad'`.lines(chomp: true)

    # Interactive
    tmux.send_keys %(printf '%s\n' aaaaa b ccc BAD | #{TUG} -q '!bad'), :Enter
    tmux.until do |lines|
  ...
  def test_disabled
    tmux.send_keys %(seq 1000 | #{TUG} --query 333 --disabled --bind a:enable-search,b:disable-search,c:toggle-search --preview 'echo {} {q}'), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
  ...
  def test_clear_query
    tmux.send_keys %(: | #{TUG} --query foo --bind space:clear-query), :Enter
  ...
  def test_change_query
    tmux.send_keys %(: | #{TUG} --query foo --bind space:change-query:foobar), :Enter
    tmux.until { |lines| assert_equal 0, lines.item_count }
    tmux.until { |lines| assert_equal '> foo', lines.last }
  ...
  def test_transform_query
    tmux.send_keys %{#{TUG} --bind 'ctrl-r:transform-query(rev <<< {q}),ctrl-u:transform-query: tr "[:lower:]" "[:upper:]" <<< {q}' --query bar}, :Enter
  ...
  def test_transform
    tmux.send_keys %{#{TUG} --bind 'focus:transform:echo "change-prompt({tug:action})"'}, :Enter
  ...
  def test_search
    tmux.send_keys %(seq 100 | #{TUG} --query 0 --bind space:search:1), :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
    tmux.send_keys :Space
    tmux.until { |lines| assert_equal 20, lines.match_count }
    tmux.send_keys '0'
  ...
  def test_transform_search
    tmux.send_keys %(seq 1000 | #{TUG} --bind 'change:transform-search:echo {q}{q}'), :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys '1'
  ...
  def test_clear_selection
    tmux.send_keys %(seq 100 | #{TUG} --multi --bind space:clear-selection), :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.send_keys :Tab
  ...
  def test_backward_delete_char_eof
    tmux.send_keys "seq 1000 | #{TUG} --bind 'bs:backward-delete-char/eof'", :Enter
    tmux.until { |lines| assert_equal '  1000/1000', lines[-2] }
    tmux.send_keys '11'
  ...
  def test_strip_xterm_osc_sequence
    %W[\x07 \x1b\\].each do |esc|
      ...
      tmux.prepare
      tmux.send_keys \
        %(echo foo bar | #{TUG} --preview '#{tempname} {2} {1}'), :Enter
  ...
  def test_keep_right
    tmux.send_keys "seq 10000 | #{TUG} --read0 --keep-right --no-multi-line --bind space:toggle-multi-line", :Enter
  ...
  def test_backward_eof
    tmux.send_keys "echo foo | #{TUG} --bind 'backward-eof:reload(seq 100)'", :Enter
  ...
  def test_change_prompt
    tmux.send_keys "#{TUG} --bind 'a:change-prompt(a> ),b:change-prompt:b> ' --query foo", :Enter
  ...
  def test_select_deselect
    tmux.send_keys "seq 3 | #{TUG} --multi --bind up:deselect+up,down:select+down", :Enter
  ...
  def test_unbind_rebind_toggle_bind
    tmux.send_keys "seq 100 | #{TUG} --bind 'c:clear-query,d:unbind(c,d),e:rebind(c,d),f:toggle-bind(c)'", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.send_keys 'ab'
  ...
  def test_scroll_off
    tmux.send_keys "seq 1000 | #{TUG} --scroll-off=3 --bind l:last", :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
  ...
  def test_scroll_off_large
    tmux.send_keys "seq 1000 | #{TUG} --scroll-off=9999", :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    height = tmux.until { |lines| lines }.first.to_i
    tmux.send_keys :PgUp
  ...
  def test_ellipsis
    tmux.send_keys 'seq 1000 | tr "\n" , | tug --ellipsis=SNIPSNIP -e -q500', :Enter
    tmux.until { |lines| assert_equal 1, lines.match_count }
    tmux.until { |lines| assert_match(/^> SNIPSNIP.*SNIPSNIP$/, lines[-3]) }
  end

  def test_start_event
    tmux.send_keys 'seq 100 | tug --multi --sync --preview-window hidden:border-none --bind "start:select-all+last+preview(echo welcome)"', :Enter
    tmux.until do |lines|
  ...
  def test_focus_event
    tmux.send_keys 'seq 100 | tug --bind "focus:transform-prompt(echo [[{}]]),?:unbind(focus)"', :Enter
  ...
  def test_result_event
    tmux.send_keys '(echo 0; seq 10) | tug --bind "result:pos(2)"', :Enter
  ...
  def test_labels_center
    tmux.send_keys 'echo x | tug --border --border-label foobar --preview : --preview-label barfoo --bind "space:change-border-label(foobarfoo)+change-preview-label(barfoobar),enter:transform-border-label(echo foo{}foo)+transform-preview-label(echo bar{}bar)"', :Enter
    tmux.until do
  ...
  def test_labels_left
    tmux.send_keys ': | tug --border rounded --preview-window border-rounded --border-label foobar --border-label-pos 2 --preview : --preview-label barfoo --preview-label-pos 2', :Enter
    tmux.until do
      assert_includes(it[0], '╭foobar─')
      assert_includes(it[1], '╭barfoo─')
    end
  ...
  def test_labels_bottom
    tmux.send_keys ': | tug --border rounded --preview-window border-rounded --border-label foobar --border-label-pos 2:bottom --preview : --preview-label barfoo --preview-label-pos -2:bottom', :Enter
    tmux.until do
  ...
  def test_labels_variables
    tmux.send_keys ': | tug --border --border-label foobar --preview "echo \$TUG_BORDER_LABEL // \$TUG_PREVIEW_LABEL" --preview-label barfoo --bind "space:change-border-label(barbaz)+change-preview-label(bazbar)+refresh-preview,enter:transform-border-label(echo 123)+transform-preview-label(echo 456)+refresh-preview"', :Enter
    tmux.until do
      assert_includes(it[0], '─foobar─')
      assert_includes(it[1], '─barfoo─')
  ...
  def test_info_separator_unicode
    tmux.send_keys 'seq 100 | tug -q55', :Enter
    tmux.until { assert_includes(it[-2], '  1/100 ─') }
  end

  def test_info_separator_no_unicode
    tmux.send_keys 'seq 100 | tug -q55 --no-unicode', :Enter
    tmux.until { assert_includes(it[-2], '  1/100 -') }
  end

  def test_info_separator_repeat
    tmux.send_keys 'seq 100 | tug -q55 --separator _-', :Enter
    tmux.until { assert_includes(it[-2], '  1/100 _-_-') }
  end

  def test_info_separator_ansi_colors_and_tabs
    tmux.send_keys "seq 100 | tug -q55 --tabstop 4 --separator $'\\x1b[33ma\\tb'", :Enter
    tmux.until { assert_includes(it[-2], '  1/100 a   ba   ba') }
  end

  def test_info_no_separator
    tmux.send_keys 'seq 100 | tug -q55 --no-separator', :Enter
    tmux.until { assert_operator(it[-2], :==, '  1/100') }
  end

  def test_info_right
    tmux.send_keys "#{TUG} --info=right --separator x --bind 'start:reload:seq 100; sleep 10'", :Enter
    tmux.until { assert_match(%r{xxx [⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏] 100/100}, it[-2]) }
  end

  def test_info_inline_right
    tmux.send_keys "#{TUG} --info=inline-right --bind 'start:reload:seq 100; sleep 10'", :Enter
    tmux.until { assert_match(%r{[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏] 100/100}, it[-1]) }
  end

  def test_info_inline_right_clearance
    tmux.send_keys "seq 100000 | #{TUG} --info inline-right", :Enter
  ...
  def test_info_command
    tmux.send_keys(%(seq 10000 | #{TUG} --separator x --info-command 'echo -e "--\\x1b[33m$TUG_POS\\x1b[m/$TUG_INFO--"'), :Enter)
  ...
  def test_info_command_inline
    tmux.send_keys(%(seq 10000 | #{TUG} --separator x --info-command 'echo -e "--\\x1b[33m$TUG_POS\\x1b[m/$TUG_INFO--"' --info inline:xx), :Enter)
    tmux.until { assert_match(%r{^>  xx--1/10000/10000-- xx}, it[-1]) }
  end

  def test_info_command_right
    tmux.send_keys(%(seq 10000 | #{TUG} --separator x --info-command 'echo -e "--\\x1b[33m$TUG_POS\\x1b[m/$TUG_INFO--"' --info right), :Enter)
    tmux.until { assert_match(%r{xx --1/10000/10000-- *$}, it[-2]) }
  end

  def test_info_command_inline_right
    tmux.send_keys(%(seq 10000 | #{TUG} --info-command 'echo -e "--\\x1b[33m$TUG_POS\\x1b[m/$TUG_INFO--"' --info inline-right), :Enter)
    tmux.until { assert_match(%r{   --1/10000/10000-- *$}, it[-1]) }
  end

  def test_info_command_and_focus
    tmux.send_keys(%(seq 100 | #{TUG} --separator x --info-command 'echo $TUG_POS' --bind focus:clear-query), :Enter)
  ...
  def test_prev_next_selected
    tmux.send_keys 'seq 10 | tug --multi --bind ctrl-n:next-selected,ctrl-p:prev-selected', :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
  ...
  def test_track
    tmux.send_keys "seq 1000 | #{TUG} --query 555 --track --bind t:toggle-track", :Enter
    tmux.until do |lines|
  ...
  def test_track_action
    tmux.send_keys "seq 1000 | #{TUG} --query 555 --bind t:track", :Enter
    tmux.until do |lines|
      assert_equal 1, lines.match_count
      assert_includes lines, '> 555'
    end
  ...
  def test_one_and_zero
    tmux.send_keys "seq 10 | #{TUG} --bind 'zero:preview(echo no match),one:preview(echo {} is the only match)'", :Enter
    tmux.send_keys '1'
    tmux.until do |lines|
  ...
  def test_height_range_with_exit_0
    tmux.send_keys "seq 10 | #{TUG} --height ~10% --exit-0", :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
    tmux.send_keys :c
    tmux.until { |lines| assert_equal 0, lines.match_count }
  end
  ...
  def test_delete_with_modifiers
    ...
    tmux.send_keys "seq 100 | #{TUG} --bind 'ctrl-delete:up+up,shift-delete:down,focus:transform-prompt:echo [{}]'", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
  ...
  def test_tug_pos
    tmux.send_keys "seq 100 | #{TUG} --preview 'echo $TUG_POS / $TUG_MATCH_COUNT'", :Enter
  ...
  def test_change_nth
    ...
    writelines(input)
    nths = '1,2..4,-1,-3..,..2'
    tmux.send_keys %(#{TUG} -qfoo -n#{nths} --bind 'space:change-nth(2|3|4|5|),result:transform-prompt:echo "[$TUG_NTH] "' < #{tempname}), :Enter
  ...
  def test_env_vars
    def to_vars(lines)
      lines.select { it.start_with?('TUG_') }.to_h do
        key, val = it.split('=', 2)
        [key.to_sym, val]
      end
    end

    tmux.send_keys %(seq 100 | #{TUG} --multi --reverse --preview-window up,99%,noborder --preview 'env | grep ^TUG_ | sort' --no-input --bind enter:show-input+refresh-preview,space:disable-search+refresh-preview), :Enter
    expected = {
      TUG_TOTAL_COUNT: '100',
      TUG_MATCH_COUNT: '100',
      TUG_SELECT_COUNT: '0',
    ...
    tmux.send_keys :Enter
    tmux.until do |lines|
      expected.merge!(TUG_INPUT_STATE: 'enabled', TUG_ACTION: 'show-input', TUG_KEY: 'enter')
      assert_equal expected, to_vars(lines).slice(*expected.keys)
    end
    tmux.send_keys :Tab, :Tab
    tmux.until do |lines|
      expected.merge!(TUG_ACTION: 'toggle-down', TUG_KEY: 'tab', TUG_POS: '3', TUG_SELECT_COUNT: '2')
      assert_equal expected, to_vars(lines).slice(*expected.keys)
    end
    tmux.send_keys '99'
    tmux.until do |lines|
      expected.merge!(TUG_ACTION: 'char', TUG_KEY: '9', TUG_QUERY: '99', TUG_MATCH_COUNT: '1', TUG_POS: '1')
      assert_equal expected, to_vars(lines).slice(*expected.keys)
    end
    tmux.send_keys :Space
    tmux.until do |lines|
      expected.merge!(TUG_INPUT_STATE: 'disabled', TUG_ACTION: 'disable-search', TUG_KEY: 'space')
      assert_equal expected, to_vars(lines).slice(*expected.keys)
    end
  end

  def test_abort_action_chain
    tmux.send_keys %(seq 100 | #{TUG} --bind 'load:accept+up+up' > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
      assert_equal '1', File.read(tempname).chomp
    end
    tmux.send_keys %(seq 100 | #{TUG} --bind 'load:abort+become(echo {})' > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
  ...
  def test_exclude_multi
    tmux.send_keys %(seq 1000 | #{TUG} --multi --bind 'a:exclude-multi,b:reload(seq 1000),c:reload-sync(seq 1000)'), :Enter
  ...
  def test_exclude
    tmux.send_keys %(seq 1000 | #{TUG} --multi --bind 'a:exclude,b:reload(seq 1000),c:reload-sync(seq 1000)'), :Enter
  ...
  def test_accept_nth
    tmux.send_keys %((echo "foo  bar  baz"; echo "bar baz  foo") | #{TUG} --multi --accept-nth 2,2 --sync --bind start:select-all+accept > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
  ...
  def test_accept_nth_string_delimiter
    tmux.send_keys %(echo "foo  ,bar,baz" | #{TUG} -d, --accept-nth 2,2,1,3,1 --sync --bind start:accept > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
  ...
  def test_accept_nth_regex_delimiter
    tmux.send_keys %(echo "foo  :,:bar,baz" | #{TUG} --delimiter='[:,]+' --accept-nth 2,2,1,3,1 --sync --bind start:accept > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
      # Last delimiter and the whitespaces are removed
      assert_equal ['bar,bar,foo  :,:bazfoo'], File.readlines(tempname, chomp: true)
  ...
  def test_accept_nth_regex_delimiter_strip_last
    tmux.send_keys %((echo "foo:,bar:,baz"; echo "foo:,bar:,baz:,qux:,") | #{TUG} --multi --delimiter='[:,]+' --accept-nth 2.. --sync --bind 'load:select-all+accept' > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
      # Last delimiter and the whitespaces are removed
  ...
  def test_accept_nth_template
    tmux.send_keys %(echo "foo  ,bar,baz" | #{TUG} -d, --accept-nth '[{n}] 1st: {1}, 3rd: {3}, 2nd: {2}' --sync --bind start:accept > #{tempname}), :Enter
    wait do
      assert_path_exists tempname
      # Last delimiter and the whitespaces are removed

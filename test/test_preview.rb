...
class TestPreview < TestInteractive
  def test_preview
    tmux.send_keys %(seq 1000 | sed s/^2$// | #{TUG} -m --preview 'sleep 0.2; echo {{}-{+}}' --bind ?:toggle-preview), :Enter
  ...
  def test_toggle_preview_without_default_preview_command
    tmux.send_keys %(seq 100 | #{TUG} --bind 'space:preview(echo [{}]),enter:toggle-preview' --preview-window up,border-double), :Enter
    tmux.until do |lines|
  ...
  def test_show_and_hide_preview
    tmux.send_keys %(seq 100 | #{TUG} --preview-window hidden,border-bold --preview 'echo [{}]' --bind 'a:show-preview,b:hide-preview'), :Enter
  ...
  def test_preview_hidden
    tmux.send_keys %(seq 1000 | #{TUG} --preview 'echo {{}-{}-$TUG_PREVIEW_LINES-$TUG_PREVIEW_COLUMNS}' --preview-window down:1:hidden --bind ?:toggle-preview), :Enter
  ...
  def test_preview_size_0
    tmux.send_keys %(seq 100 | #{TUG} --reverse --preview 'echo {} >> #{tempname}; echo ' --preview-window 0 --bind space:toggle-preview), :Enter
    tmux.until do |lines|
      assert_equal 100, lines.match_count
  ...
  def test_preview_size_0_hidden
    tmux.send_keys %(seq 100 | #{TUG} --reverse --preview 'echo {} >> #{tempname}; echo ' --preview-window 0,hidden --bind space:toggle-preview), :Enter
  ...
  def test_preview_flags
    tmux.send_keys %(seq 10 | sed 's/^/:: /; s/$/  /' |
        #{TUG} --multi --preview 'echo {{2}/{s2}/{+2}/{+s2}/{q}/{n}/{+n}}'), :Enter
  ...
  def test_preview_file
    tmux.send_keys %[(echo foo bar; echo bar foo) | #{TUG} --multi --preview 'cat {+f} {+f2} {+nf} {+fn}' --print0], :Enter
  ...
  def test_preview_q_no_match
    tmux.send_keys %(: | #{TUG} --preview 'echo foo {q} foo'), :Enter
  ...
  def test_preview_q_no_match_with_initial_query
    tmux.send_keys %(: | #{TUG} --preview 'echo 1. /{q}/{q:1}/; echo 2. /{q:..}/{q:2}/{q:-1}/; echo 3. /{q:s-2}/{q:-2}/{q:x}/' --query 'foo bar'), :Enter
    tmux.until { |lines| assert_equal 0, lines.match_count }
  ...
  def test_preview_update_on_select
    tmux.send_keys %(seq 10 | tug -m --preview 'echo {+}' --bind a:toggle-all),
                   :Enter
  ...
  def test_preview_correct_tab_width_after_ansi_reset_code
    ...
    tmux.send_keys "#{TUG} --preview 'cat #{tempname}'", :Enter
    tmux.until { |lines| assert_includes lines[1], ' +       green ' }
  end

  def test_preview_bindings_with_default_preview
    tmux.send_keys "seq 10 | #{TUG} --preview 'echo [{}]' --bind 'a:preview(echo [{}{}]),b:preview(echo [{}{}{}]),c:refresh-preview'", :Enter
  ...
  def test_preview_bindings_without_default_preview
    tmux.send_keys "seq 10 | #{TUG} --bind 'a:preview(echo [{}{}]),b:preview(echo [{}{}{}]),c:refresh-preview'", :Enter
    tmux.until { |lines| lines.match_count == 10 }
    tmux.until { |lines| refute_includes lines[1], '1' }
    tmux.send_keys 'a'
    tmux.until { |lines| assert_includes lines[1], '[11]' }
  ...
  def test_preview_scroll_begin_constant
    tmux.send_keys "echo foo 123 321 | #{TUG} --preview 'seq 1000' --preview-window left:+123", :Enter
  ...
  def test_preview_scroll_begin_expr
    tmux.send_keys "echo foo 123 321 | #{TUG} --preview 'seq 1000' --preview-window left:+{3}", :Enter
    tmux.until { |lines| assert_match %r{1/1}, lines[-2] }
  ...
  def test_preview_scroll_begin_and_offset
    ['echo foo 123 321', 'echo foo :123: 321'].each do |input|
      tmux.send_keys "#{input} | #{TUG} --preview 'seq 1000' --preview-window left:+{2}-2", :Enter
      tmux.until { |lines| assert_match %r{1/1}, lines[-2] }
  ...
  def test_preview_clear_screen
    tmux.send_keys %{seq 100 | #{TUG} --preview 'for i in $(seq 300); do (( i % 200 == 0 )) && printf "\\033[2J"; echo "[$i]"; sleep 0.001; done'}, :Enter
    tmux.until { |lines| lines.match_count == 100 }
    tmux.until { |lines| lines[1]&.include?('[200]') }
  end

  def test_preview_window_follow
    file = Tempfile.new('tug-follow')
    file.sync = true

    tmux.send_keys %(seq 100 | #{TUG} --preview 'echo start; tail -f "#{file.path}"' --preview-window follow --bind 'up:preview-up,down:preview-down,space:change-preview-window:follow|nofollow' --preview-window '~4'), :Enter
    tmux.until { |lines| lines.match_count == 100 }
    ...
    # Scroll the preview window and tug should stop following the file content
    tmux.send_keys :Up
    ...
    # Scroll back to the bottom and tug should start following the file again
  ...
  def test_toggle_preview_wrap
    tmux.send_keys "#{TUG} --preview 'for i in $(seq $TUG_PREVIEW_COLUMNS); do echo -n .; done; echo wrapped; echo 2nd line' --bind ctrl-w:toggle-preview-wrap", :Enter
    2.times do
  ...
  def test_close
    tmux.send_keys "seq 100 | #{TUG} --preview 'echo foo' --bind ctrl-c:close", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
  ...
  def test_preview_header
    tmux.send_keys "seq 100 | #{TUG} --bind ctrl-k:preview-up+preview-up,ctrl-j:preview-down+preview-down+preview-down --preview 'seq 1000' --preview-window 'top:+{1}:~3'", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
  ...
  def test_change_preview_window
    tmux.send_keys "seq 1000 | #{TUG} --preview 'echo [[{}]]' --no-preview-border --bind '" \
                   ...
                   "e:preview(printf ::%${TUG_PREVIEW_COLUMNS}s{})+change-preview-window(up),f:change-preview-window(up,wrap)'", :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
  ...
  def test_change_preview_window_should_not_reset_change_preview
    tmux.send_keys "#{TUG} --preview-window up,border-none --bind 'start:change-preview(echo hello)' --bind 'enter:change-preview-window(border-left)'", :Enter
  ...
  def test_change_preview_window_rotate
    tmux.send_keys "seq 100 | #{TUG} --preview-window left,border-none --preview 'echo hello' --bind '" \
  ...
  def test_change_preview_window_rotate_hidden
    tmux.send_keys "seq 100 | #{TUG} --preview-window hidden --preview 'echo =={}==' --bind '" \
  ...
  def test_change_preview_window_rotate_hidden_down
    tmux.send_keys "seq 100 | #{TUG} --bind '?:change-preview-window:up||down|' --preview 'echo =={}==' --preview-window hidden,down,1", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.until { |lines| refute_includes lines[1], '==1==' }
    tmux.send_keys '?'
    tmux.until { |lines| assert_includes lines[1], '==1==' }
  ...
  def test_toggle_alternative_preview_window
    tmux.send_keys "seq 10 | #{TUG} --bind space:toggle-preview --preview-window '<100000(hidden,up,border-none)' --preview 'echo /{}/{}/'", :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
    tmux.until { |lines| refute_includes lines, '/1/1/' }
    tmux.send_keys :Space
    tmux.until { |lines| assert_includes lines, '/1/1/' }
  ...
  def test_alternative_preview_window_opts
    tmux.send_keys "seq 10 | #{TUG} --preview-border rounded --preview-window '~5,2,+0,<100000(~0,+100,wrap,noinfo)' --preview 'seq 1000'", :Enter
    tmux.until { |lines| assert_equal 10, lines.match_count }
    tmux.until do |lines|
  ...
  def test_preview_window_width_exception
    tmux.send_keys "seq 10 | #{TUG} --scrollbar --preview-window border-left --border --preview 'seq 1000'", :Enter
    tmux.until do |lines|
  ...
  def test_preview_window_hidden_on_focus
    tmux.send_keys "seq 3 | #{TUG} --preview 'echo {}' --bind focus:hide-preview", :Enter
    tmux.until { |lines| assert_includes lines, '> 1' }
    tmux.send_keys :Up
  ...
  def test_preview_query_should_not_be_affected_by_search
    tmux.send_keys "seq 1 | #{TUG} --bind 'change:transform-search(echo {q:1})' --preview 'echo [{q}/{}]'", :Enter

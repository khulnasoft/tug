...
module TestShell
  ...
  def test_ctrl_t
    set_var('TUG_CTRL_T_COMMAND', 'seq 100')
  ...
  def test_ctrl_t_unicode
    writelines(['tug-unicode 테스트1', 'tug-unicode 테스트2'])
    set_var('TUG_CTRL_T_COMMAND', "cat #{tempname}")

    tmux.prepare
    tmux.send_keys 'echo ', 'C-t'
    tmux.until { |lines| assert_equal 2, lines.match_count }
    tmux.send_keys 'tug-unicode'
    tmux.until { |lines| assert_equal 2, lines.match_count }
    ...
    tmux.send_keys :Enter
    tmux.until { |lines| assert_match(/echo .*tug-unicode.*1.* .*tug-unicode.*2/, lines.join) }
    tmux.send_keys :Enter
    tmux.until { |lines| assert_equal 'tug-unicode 테스트1 tug-unicode 테스트2', lines[-1] }
  end
  ...
  def test_alt_c_command
    set_var('TUG_ALT_C_COMMAND', 'echo /tmp')
...
module CompletionTest
  def test_file_completion
    FileUtils.mkdir_p('/tmp/tug-test')
    FileUtils.mkdir_p('/tmp/tug test')
    (1..100).each { |i| FileUtils.touch("/tmp/tug-test/#{i}") }
    ['no~such~user', '/tmp/tug test/foobar'].each do |f|
      FileUtils.touch(File.expand_path(f))
    end
    tmux.prepare
    tmux.send_keys 'cat /tmp/tug-test/10**', :Tab
    tmux.until { |lines| assert_operator lines.match_count, :>, 0 }
    tmux.send_keys ' !d'
    tmux.until { |lines| assert_equal 2, lines.match_count }
    ...
    tmux.until(true) do |lines|
      assert_equal 'cat /tmp/tug-test/10 /tmp/tug-test/100', lines[-1]
    end
    ...
    # /tmp/tug\ test**<TAB>
    tmux.send_keys 'C-u'
    tmux.send_keys 'cat /tmp/tug\ test/**', :Tab
    tmux.until { |lines| assert_operator lines.match_count, :>, 0 }
    tmux.send_keys 'foobar$'
    tmux.until do |lines|
      assert_equal 1, lines.match_count
      assert lines.any_include?('> /tmp/tug test/foobar')
    end
    tmux.send_keys :Enter
    tmux.until(true) { |lines| assert_equal 'cat /tmp/tug\ test/foobar', lines[-1] }

    # Should include hidden files
    (1..100).each { |i| FileUtils.touch("/tmp/tug-test/.hidden-#{i}") }
    tmux.send_keys 'C-u'
    tmux.send_keys 'cat /tmp/tug-test/hidden**', :Tab
    tmux.until(true) do |lines|
      assert_equal 100, lines.match_count
      assert lines.any_include?('/tmp/tug-test/.hidden-')
    end
    tmux.send_keys :Enter
  ensure
    ['/tmp/tug-test', '/tmp/tug test', '~/.tug-home', 'no~such~user'].each do |f|
  ...
  def test_dir_completion
    (1..100).each do |idx|
      FileUtils.mkdir_p("/tmp/tug-test/d#{idx}")
    end
    FileUtils.touch('/tmp/tug-test/d55/xxx')
    tmux.prepare
    tmux.send_keys 'cd /tmp/tug-test/**', :Tab
    tmux.until { |lines| assert_operator lines.match_count, :>, 0 }
    ...
    tmux.until do |lines|
      ...
      assert_includes lines, '> /tmp/tug-test/d55/'
    end
    tmux.send_keys :Enter
    tmux.until(true) { |lines| assert_equal 'cd /tmp/tug-test/d55/', lines[-1] }
    tmux.send_keys :xx
    tmux.until { |lines| assert_equal 'cd /tmp/tug-test/d55/xx', lines[-1] }
    ...
    tmux.until { |lines| assert_equal 'cd /tmp/tug-test/d55', lines[-1] }
    tmux.send_keys :Tab
    tmux.until { |lines| assert_equal 'cd /tmp/tug-test/d55/', lines[-1] }
  end
  ...
  def test_custom_completion
    tmux.send_keys '_tug_compgen_path() { echo "$1"; seq 10; }', :Enter
    tmux.prepare
  ...
  def test_unset_completion
    tmux.send_keys 'export TUGFOOBAR=BAZ', :Enter
    tmux.prepare

    # Using tmux
    tmux.send_keys 'unset TUGFOOBR**', :Tab
    tmux.until { |lines| assert_equal 1, lines.match_count }
    tmux.send_keys :Enter
    tmux.until { |lines| assert_equal 'unset TUGFOOBAR', lines[-1] }
    tmux.send_keys 'C-c'
  ...
  def test_completion_in_command_sequence
    ...
    triggers.each do |trigger|
      set_var('TUG_COMPLETION_TRIGGER', trigger)
      command = "echo foo; QUX=THUD unset TUGFOOBR#{trigger}"
      tmux.send_keys command.sub(/(;|`)$/, '\\\\\1'), :Tab
      tmux.until { |lines| assert_equal 1, lines.match_count }
      tmux.send_keys :Enter
      tmux.until { |lines| assert_equal 'echo foo; QUX=THUD unset TUGFOOBAR', lines[-1] }
    end
  end

  def test_file_completion_unicode
    FileUtils.mkdir_p('/tmp/tug-test')
    tmux.paste "cd /tmp/tug-test; echo test3 > $'tug-unicode \\355\\205\\214\\354\\212\\244\\355\\212\\2701'; echo test4 > $'tug-unicode \\355\\205\\214\\354\\212\\244\\355\\212\\2702'"
    tmux.prepare
    tmux.send_keys 'cat tug-unicode**', :Tab
    tmux.until { |lines| assert_equal 2, lines.match_count }
    ...
    tmux.send_keys :Enter
    tmux.until(true) { |lines| assert_match(/cat .*tug-unicode.*1.* .*tug-unicode.*2/, lines[-1]) }
    tmux.send_keys :Enter
    tmux.until { |lines| assert_equal %w[test3 test4], lines[-2..] }
  end

  def test_custom_completion_api
    tmux.send_keys 'eval "_tug$(declare -f _comprun)"', :Enter
    ...
    tmux.prepare
    tmux.send_keys 'unset -f _tug_comprun', :Enter
  end

  def test_ssh_completion
    (1..5).each { |i| FileUtils.touch("/tmp/tug-test-ssh-#{i}") }
    ...
    tmux.send_keys ' -i /tmp/tug-test-ssh**', :Tab
    tmux.until do |lines|
    ...
    tmux.send_keys :Enter
    tmux.until { |lines| assert lines.any_include?('ssh jg@localhost  -i /tmp/tug-test-ssh-') }
...
class TestBash < TestBase
  ...
  def new_shell
    tmux.prepare
    tmux.send_keys "TUG_TMUX=1 #{Shell.bash}", :Enter
    tmux.prepare
  end

  def test_dynamic_completion_loader
    tmux.paste 'touch /tmp/foo; _tug_completion_loader=1'
    tmux.paste '_completion_loader() { complete -o default fake; }'
    tmux.paste 'complete -F _tug_path_completion -o default -o bashdefault fake'
...
class TestZsh < TestBase
  ...
  def new_shell
    tmux.send_keys "TUG_TMUX=1 #{Shell.zsh}", :Enter
    tmux.prepare
  end
  ...
  def test_complete_quoted_command
    ...
    ['unset', '\unset', "'unset'"].each do |command|
      tmux.prepare
      tmux.send_keys "#{command} TUGFOOBR**", :Tab
      tmux.until { |lines| assert_equal 1, lines.match_count }
      tmux.send_keys :Enter
      tmux.until { |lines| assert_equal "#{command} TUGFOOBAR", lines[-1] }
      tmux.send_keys 'C-c'
    end
  end
end
...
class TestFish < TestBase
  ...
  def new_shell
    tmux.send_keys 'env TUG_TMUX=1 TUG_DEFAULT_OPTS=--no-scrollbar fish', :Enter

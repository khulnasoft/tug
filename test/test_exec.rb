...
class TestExec < TestInteractive
  def test_execute
    ...
    tmux.send_keys "cat #{tempname} | #{TUG} #{opts}", :Enter
  ...
  def test_execute_multi
    output = '/tmp/tug-test-execute-multi'
  ...
  def test_execute_plus_flag
    ...
    tmux.send_keys "cat #{tempname} | #{TUG} --multi --bind 'x:execute-silent(echo {+}/{}/{+2}/{2} >> #{output})'", :Enter
  ...
  def test_execute_shell
    ...
    tmux.send_keys "echo foo | SHELL=#{tempname} tug --bind 'enter:execute:{}bar'", :Enter
  ...
  def test_interrupt_execute
    tmux.send_keys "seq 100 | #{TUG} --bind 'ctrl-l:execute:echo executing {}; sleep 100'", :Enter
  ...
  def test_kill_default_command_on_abort
    ...
    tmux.send_keys TUG.sub('TUG_DEFAULT_COMMAND=', "TUG_DEFAULT_COMMAND=#{tempname}"), :Enter
    tmux.until { |lines| assert_equal 1, lines.match_count }
    tmux.send_keys 'C-c'
  ...
  def test_kill_default_command_on_accept
    ...
    tmux.until { |lines| assert_equal 1, lines.match_count }
    tmux.send_keys :Enter
    assert_equal 'Started', tug_output
    wait { refute system("pgrep -f #{tempname}") }
  ensure
    system("pkill -9 -f #{tempname}")
  end
  ...
  def test_kill_reload_command_on_abort
    ...
    tmux.send_keys "seq 1 3 | #{TUG} --bind 'ctrl-r:reload(#{tempname})'", :Enter
    tmux.until { |lines| assert_equal 3, lines.match_count }
    tmux.send_keys 'C-r'
    tmux.until { |lines| assert_equal 1, lines.match_count }
    tmux.send_keys 'C-c'
  ...
  def test_reload
    tmux.send_keys %(seq 1000 | #{TUG} --bind 'change:reload(seq $TUG_QUERY),a:reload(seq 100),b:reload:seq 200' --header-lines 2 --multi 2), :Enter
  ...
  def test_reload_even_when_theres_no_match
    tmux.send_keys %(: | #{TUG} --bind 'space:reload(seq 10)'), :Enter
  ...
  def test_reload_should_terminate_standard_input_stream
    tmux.send_keys %(ruby -e "STDOUT.sync = true; loop { puts 1; sleep 0.1 }" | tug --bind 'start:reload(seq 100)'), :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
  end

  def test_clear_list_when_header_lines_changed_due_to_reload
    tmux.send_keys %(seq 10 | #{TUG} --header 0 --header-lines 3 --bind 'space:reload(seq 1)'), :Enter
  ...
  def test_item_index_reset_on_reload
    tmux.send_keys "seq 10 | #{TUG} --preview 'echo [[{n}]]' --bind 'up:last,down:first,space:reload:seq 100'", :Enter
  ...
  def test_reload_should_update_preview
    tmux.send_keys "seq 3 | #{TUG} --bind 'ctrl-t:reload:echo 4' --preview 'echo {}' --preview-window 'nohidden'", :Enter
  ...
  def test_reload_and_change_preview_should_update_preview
    tmux.send_keys "seq 3 | #{TUG} --bind 'ctrl-t:reload(echo 4)+change-preview(echo {})'", :Enter
    tmux.until { |lines| assert_equal 3, lines.match_count }
  ...
  def test_reload_sync
    tmux.send_keys "seq 100 | #{TUG} --bind 'load:reload-sync(sleep 1; seq 1000)+unbind(load)'", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.send_keys '00'
    tmux.until { |lines| assert_equal 1, lines.match_count }
    # After 1 second
  ...
  def test_reload_disabled_case1
    tmux.send_keys "seq 100 | #{TUG} --query 99 --bind 'space:disable-search+reload(sleep 2; seq 1000)'", :Enter
    tmux.until do |lines|
  ...
  def test_reload_disabled_case2
    tmux.send_keys "seq 100 | #{TUG} --query 99 --bind 'space:disable-search+reload-sync(sleep 2; seq 1000)'", :Enter
    tmux.until do |lines|
      assert_equal 100, lines.item_count
      assert_equal 1, lines.match_count
    end
  ...
  def test_reload_disabled_case3
    tmux.send_keys "seq 100 | #{TUG} --query 99 --bind 'space:disable-search+reload(sleep 2; seq 1000)+backward-delete-char'", :Enter
    tmux.until do |lines|
      assert_equal 100, lines.item_count
      assert_equal 1, lines.match_count
    end
  ...
  def test_reload_disabled_case4
    tmux.send_keys "seq 100 | #{TUG} --query 99 --bind 'space:disable-search+reload-sync(sleep 2; seq 1000)+backward-delete-char'", :Enter
    tmux.until do |lines|
      assert_equal 100, lines.item_count
      assert_equal 1, lines.match_count
    end
  ...
  def test_reload_disabled_case5
    tmux.send_keys "seq 100 | #{TUG} --query 99 --bind 'space:disable-search+reload(echo xx; sleep 2; seq 1000)'", :Enter
    tmux.until do |lines|
      assert_equal 100, lines.item_count
      assert_equal 1, lines.match_count
    end
  ...
  def test_reload_disabled_case6
    tmux.send_keys "seq 1000 | #{TUG} --disabled --bind 'change:reload:sleep 0.5; seq {q}'", :Enter
    tmux.until { |lines| assert_equal 1000, lines.match_count }
    tmux.send_keys '9'
  ...
  def test_reload_and_change
    tmux.send_keys "(echo foo; echo bar) | #{TUG} --bind 'load:reload-sync(sleep 60)+change-query(bar)'", :Enter
    tmux.until { |lines| assert_equal 1, lines.match_count }
  end

  def test_become_tty
    tmux.send_keys "sleep 0.5 | #{TUG} --bind 'start:reload:ls' --bind 'load:become:tty'", :Enter
    tmux.until { |lines| assert_includes lines, '/dev/tty' }
  end

  def test_disabled_preview_update
    tmux.send_keys "echo bar | #{TUG} --disabled --bind 'change:reload:echo foo' --preview 'echo [{q}-{}]'", :Enter
    tmux.until { |lines| assert_equal 1, lines.match_count }
  ...
  def test_start_on_reload
    tmux.send_keys %(echo foo | #{TUG} --header Loading --header-lines 1 --bind 'start:reload:sleep 2; echo bar' --bind 'load:change-header:Loaded' --bind space:change-header:), :Enter
  ...
  def test_become
    tmux.send_keys "seq 100 | #{TUG} --bind 'enter:become:seq {} | #{TUG}'", :Enter
    tmux.until { |lines| assert_equal 100, lines.match_count }
    tmux.send_keys 999
    tmux.until { |lines| assert_equal 0, lines.match_count }
    tmux.send_keys :Enter
